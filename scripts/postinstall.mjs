#!/usr/bin/env node
/**
 * Post-install warm-up for Birouter native runtimes.
 *
 * Runs after the main `scripts/build/postinstall.mjs` binary-copy step.
 * Tries to pre-resolve better-sqlite3 into ~/.birouter/runtime/ so that
...
if (
  process.env.BIROUTER_SKIP_POSTINSTALL === "1" ||
  process.env.CI === "true" ||
  process.env.CI === "1"
) {
  process.exit(0);
...
(async () => {
  try {
    ...
    console.warn(`[birouter] postinstall warm-up skipped: ${err?.message ?? err}`);
  }
})();
