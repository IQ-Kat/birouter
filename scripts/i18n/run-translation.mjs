#!/usr/bin/env node
/**
 * Birouter — Docs translation pipeline (hash-based, incremental).
 *
 ...
 *   BIROUTER_TRANSLATION_API_URL     e.g. https://cloud.birouter.dev/v1
 *   BIROUTER_TRANSLATION_API_KEY     bearer token (kept out of logs)
 *   BIROUTER_TRANSLATION_MODEL       e.g. cx/gpt-5.4-mini
 *   BIROUTER_TRANSLATION_TIMEOUT_MS  optional, default 60000
 *   BIROUTER_TRANSLATION_CONCURRENCY optional, default 4
 */
...
function backendConfig() {
  const apiUrl = requireEnv("BIROUTER_TRANSLATION_API_URL").replace(/\/$/, "");
  const apiKey = requireEnv("BIROUTER_TRANSLATION_API_KEY");
  const model = requireEnv("BIROUTER_TRANSLATION_MODEL");
  const timeoutMs = Number(process.env.BIROUTER_TRANSLATION_TIMEOUT_MS || 60000);
...
async function main() {
  ...
  if (!opts.dryRun) {
    ...
    else backend.concurrency = Number(process.env.BIROUTER_TRANSLATION_CONCURRENCY || 4);
    logInfo(
      `backend: ${backend.apiUrl} (model=${backend.model}, concurrency=${backend.concurrency}, timeout=${backend.timeoutMs}ms)`
    );
  } else {
    const apiUrl = (process.env.BIROUTER_TRANSLATION_API_URL || "").replace(/\/$/, "");
