...
whose source SHA-256 changed), runs a Cloud LLM through
BIROUTER_TRANSLATION_* env vars, and is wired into `npm run i18n:run`
...
def translate_block(text, target_language, api_url, api_key, model):
    ...
    prompt = (
        f"You are a professional technical translator working on the Birouter proxy project documentation.\n"
    ...
    data = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "You are a direct translator. Output only the requested translation.",
            },
...
def process_file(file_path, target_language, api_url, api_key, model):
    ...
    if needs_update:
        ...
        print(
            f"  ⏩ {file_path.name} already fully translated or no English blocks found."
        )


def main():
    parser = argparse.ArgumentParser(
        description="Birouter Auto-Translator for i18n Markdown"
    )
    parser.add_argument(
        "--api-url",
    ...
    parser.add_argument(
        "--model", default="gemini/gemini-3-flash", help="Model name to use"
    )
    parser.add_argument(
        "--lang",
        default=None,
        help="Process only a specific language code (e.g. pt-BR)",
    )
    ...
    if args.lang:
        lang_dirs = [
            d for d in I18N_DIR.iterdir() if d.is_dir() and d.name == args.lang
        ]
    else:
