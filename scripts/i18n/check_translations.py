#!/usr/bin/env python3
"""
Translation check script for Birouter.
    ...
    for root, dirs, files in os.walk(src_dir):
        for f in files:
            ...
            try:
                ...
                matches = re.findall(
                    r'useTranslations\(["\']+([^"\']+)["\']+\)', content
                )
                used_ns.update(matches)
            except (IOError, UnicodeDecodeError) as e:
                print(
                    f"Warning: could not process file {os.path.join(root, f)}: {e}",
                    file=sys.stderr,
                )
    return used_ns
...
def check_translations(
    src_dir="src", en_json_path="src/i18n/messages/en.json", verbose=False
):
...
def main():
    ...
    parser.add_argument(
        "--json", default="src/i18n/messages/en.json", help="Path to en.json"
    )
