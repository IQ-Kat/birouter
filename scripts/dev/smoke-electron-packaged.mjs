...
const DEFAULT_URL = "http://127.0.0.1:2004/login";
export const LINUX_EXECUTABLE_NAMES = ["birouter-desktop", "birouter", "Birouter"];
...
function discoverMacExecutable() {
  ...
  const candidates = [
    ...
    join(distDir, "mac", "Birouter.app", "Contents", "MacOS", "Birouter"),
    join(distDir, "mac-arm64", "Birouter.app", "Contents", "MacOS", "Birouter"),
  ];
...
function discoverWindowsExecutable() {
  const distDir = join(ROOT, "electron", "dist-electron");
  const candidates = [
    join(distDir, "win-unpacked", "Birouter.exe"),
    join(distDir, "win-x64-unpacked", "Birouter.exe"),
    join(distDir, "win-arm64-unpacked", "Birouter.exe"),
  ];

  return (
    candidates.find((candidate) => existsSync(candidate)) ||
    findExecutableByName(distDir, ["Birouter.exe"]) ||
    candidates[0]
  );
}
...
async function assertPortIsFree(url) {
  try {
    ...
    throw new Error(
      `Smoke URL already responded with HTTP ${response.status}: ${url}. Stop the existing Birouter process before running this check.`
    );
  } catch (error) {
...
async function ensureSmokeEnvDirs(smokeEnv, dataDir) {
  ...
  if (platform() === "win32" && smokeEnv.APPDATA) {
    for (const subdir of ["birouter-desktop", "Birouter", "birouter"]) {
...
async function main() {
  ...
  if (!existsSync(appExecutable)) {
    throw new Error(
      `Packaged Birouter executable not found at ${appExecutable}. Build it first with \`npm run build:<target> --prefix electron\` or set ELECTRON_SMOKE_APP_EXECUTABLE.`
    );
  }
  ...
  const dataDir =
    ...
    (await mkdtemp(join(tmpdir(), "birouter-electron-smoke-")));
  const removeDataDir =
