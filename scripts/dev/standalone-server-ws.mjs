...
// Opt-in native HTTPS (#5242). Resolved once at boot: when both BIROUTER_TLS_CERT
// and BIROUTER_TLS_KEY point at readable files we terminate TLS on the same
...
if (tlsOptions) {
  console.log(`[birouter][tls] HTTPS enabled — terminating TLS with cert=${tlsOptions.certPath}`);
}
