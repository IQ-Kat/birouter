...
const CRYPTO_SECRETS = {
  ...
  // Generated in bin/birouter.mjs:ensureStorageEncryptionKey() and persisted to
  // ~/.birouter/.env to survive across upgrades. This prevents credential loss
  // when upgrading Birouter (issue #1622).
  MACHINE_ID_SALT: () => `birouter-${randomBytes(8).toString("hex")}`,
};
 ...
 * It's generated at server startup in bin/birouter.mjs and persisted to
 * ~/.birouter/.env to survive across upgrades.
...
function resolveDataDir(env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return join(appData, "birouter");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return join(resolve(xdg), "birouter");

  return join(homedir(), ".birouter");
}
        ...
        if (ENCRYPTION_BOUND_KEYS.has(key) && dbHasEncrypted) {
          log(
            `⚠️  ${key} NOT generated — encrypted credentials exist in DB. ` +
              `Restore your previous key via ~/.birouter/server.env, ~/.birouter/.env, ` +
