#!/usr/bin/env node
/**
 * system-info.mjs — Birouter System Information Reporter (#280)
 *
...
lines.push("Birouter System Information Report");
...
const installedGlobal = run("npm list -g birouter --depth=0 2>/dev/null | grep birouter");
lines.push(`Global npm: ${installedGlobal || "not installed globally"}`);

const pm2Status = run("pm2 list 2>/dev/null | grep birouter | awk '{print $4, $10, $12}'");
...
console.log(
  `📎 Attach this file when reporting issues at: https://github.com/IQ-Kat/birouter/issues`
);
