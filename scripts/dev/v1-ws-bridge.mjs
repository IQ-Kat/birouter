...
function normalizeEndpoint(rawEndpoint) {
  ...
  let parsed;
  try {
    parsed = new URL(endpoint, "http://birouter.local");
  } catch {
    return null;
  }

  if (parsed.origin !== "http://birouter.local") {
    return null;
  }
...
class WebSocketSession {
  ...
  async executeRequest(requestId, endpoint, payload, abortController) {
    const headers = {
      ...
      "content-type": "application/json",
      "x-birouter-ws-session-id": this.sessionId,
      "x-birouter-ws-request-id": requestId,
    };
...
export function createBirouterWsBridge({
  baseUrl,
  fetchImpl = fetch,
  ...
  if (!isText(baseUrl)) {
    throw new Error("createBirouterWsBridge requires a baseUrl");
  }
