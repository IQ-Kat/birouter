#!/usr/bin/env node
/**
 * Birouter — Zero-Config Bootstrap
 *
 ...
 *   3. Preferred config .env  (DATA_DIR/.env -> ~/.birouter/.env -> ./.env)
...
function writeEnvFile(filePath, env) {
  const lines = [
    "# Auto-generated by Birouter bootstrap — do not delete",
...
export function bootstrapEnv({ dataDirOverride, quiet = false } = {}) {
  ...
  const envCandidates = [];
  envCandidates.push(join(resolveDataDir(dataDirOverride, process.env), ".env"));
  envCandidates.push(join(process.cwd(), ".env"));
  if (process.env.DATA_DIR?.trim()) {
    envCandidates.push(join(resolve(process.env.DATA_DIR.trim()), ".env"));
  }
  ...
  for (const filePath of envCandidates) {
    if (existsSync(filePath)) {
      preferredEnv = { ...preferredEnv, ...parseEnvFile(filePath) };
    }
  }
  ...
  // This keeps run-next / run-standalone consistent with `bin/birouter.mjs`.
  //
  ...
  if (needsPersist) {
    merged.BIROUTER_BOOTSTRAPPED = "true";
  }
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // Legacy salt (fallback): scryptSync(secret, "birouter-field-encryption-v1", 32)
              const legacySalt = "birouter-field-encryption-v1";
              ...
              if (!keyMatched) {
                ...
                log(
                  "   Either restore your previous key via ~/.birouter/server.env or ~/.birouter/.env,"
                );
                log(
                  "   or run: birouter reset-encrypted-columns --force  (wipes credentials, keeps provider config)"
                );
              }
            }
          }
