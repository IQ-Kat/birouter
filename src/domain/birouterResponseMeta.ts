...
export function getBirouterTokenCounts(usage: UsageLike): { input: number; output: number } {
...
export function formatBirouterCost(costUsd: unknown): string {
  const normalized = toFiniteNumber(costUsd);
  return normalized > 0 ? normalized.toFixed(10) : "0.0000000000";
}

export function buildBirouterResponseMetaHeaders({
  cacheHit = false,
  costUsd = 0,
   ...
   * Emitted as `X-Birouter-Cost-Saved` only when provided (omitted on normal
  ...
  const tokens = getBirouterTokenCounts(usage);
  const headers: Record<string, string> = {
    [BIROUTER_RESPONSE_HEADERS.cacheHit]: toHeaderValue(String(cacheHit)),
    [BIROUTER_RESPONSE_HEADERS.latencyMs]: toHeaderValue(String(toNonNegativeInteger(latencyMs))),
    [BIROUTER_RESPONSE_HEADERS.responseCost]: toHeaderValue(formatBirouterCost(costUsd)),
    [BIROUTER_RESPONSE_HEADERS.tokensIn]: toHeaderValue(String(tokens.input)),
    [BIROUTER_RESPONSE_HEADERS.tokensOut]: toHeaderValue(String(tokens.output)),
    [BIROUTER_RESPONSE_HEADERS.version]: toHeaderValue(APP_CONFIG.version),
  };

  if (typeof model === "string" && model.trim().length > 0) {
    headers[BIROUTER_RESPONSE_HEADERS.model] = toHeaderValue(model);
  }

  if (typeof requestId === "string" && requestId.trim().length > 0) {
    headers[BIROUTER_RESPONSE_HEADERS.requestId] = toHeaderValue(requestId);
  }

  if (typeof provider === "string" && provider.trim().length > 0) {
    headers[BIROUTER_RESPONSE_HEADERS.provider] = toHeaderValue(getProviderAlias(provider));
  }
  ...
  if (costSavedUsd != null) {
    headers[BIROUTER_RESPONSE_HEADERS.costSaved] = toHeaderValue(formatBirouterCost(costSavedUsd));
  }

  const attempts = toNonNegativeInteger(fallbackAttempts);
  if (attempts > 0) {
    headers[BIROUTER_RESPONSE_HEADERS.fallbackAttempts] = toHeaderValue(String(attempts));
  }

  return headers;
}

export function buildBirouterSseMetadataComment(
  options: Parameters<typeof buildBirouterResponseMetaHeaders>[0]
): string {
  const headers = buildBirouterResponseMetaHeaders(options);
...
/**
 * Single choke-point for attaching the X-Birouter-* response meta headers.
 ...
 */
export function attachBirouterMetaHeaders(
  headers: Headers | Record<string, string>,
  meta: Parameters<typeof buildBirouterResponseMetaHeaders>[0]
): void {
  const built = buildBirouterResponseMetaHeaders(meta);
...
/**
 * Attach the X-Birouter-* meta headers onto an already-built Response, ADDING
 ...
 */
export function attachBirouterMetaToResponse(
  response: Response,
  meta: Parameters<typeof buildBirouterResponseMetaHeaders>[0]
): Response {
  ...
  try {
    attachBirouterMetaHeaders(response.headers, meta);
    return response;
  } catch {
    ...
    attachBirouterMetaHeaders(cloned.headers, meta);
    return cloned;
  }
}
