...
// ─── Provider mapping: LiteLLM provider → Birouter aliases ─────
...
/**
 * Transform LiteLLM raw data → Birouter PricingByProvider format.
 *
 * Conversion: cost_per_token × 1_000_000 → $/1M tokens (Birouter format).
 ...
 */
export function transformToBirouter(raw: Record<string, LiteLLMModelInfo>): PricingByProvider {
  ...
  for (const [modelKey, info] of Object.entries(raw)) {
    ...
    // Map to Birouter providers
    const litellmProvider = info.litellm_provider || "";
    const biRouteProviders = LITELLM_PROVIDER_MAP[litellmProvider];

    if (biRouteProviders) {
      for (const provider of biRouteProviders) {
  ...
  try {
    ...
    for (const source of validSources) {
      if (source === "litellm") {
        ...
        const transformed = transformToBirouter(raw);
