 ...
 *   model: string;                    // the model name the user chose from Birouter
 * }
 *
 * interface HermesAgentConfigPayload {
 *   baseUrl: string;                  // usually the Birouter base URL
...
export async function generateHermesAgentConfig(
  ...
  const resolvedKey = apiKey || "YOUR_BIROUTER_API_KEY_HERE";
  ...
  // Build the providers.birouter entry (shared)
  const normalizedBase = normalizeBaseUrl(baseUrl);
  const birouterProvider = {
