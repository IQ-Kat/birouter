...
// Collect PIDs of all birouter-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from birouter install dir or wrapper cli.js
        const isAppProcess = lower.includes("birouter") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("birouter") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "birouter");
  }
  return path.join(os.homedir(), ".birouter");
}
...
// Resolve npx/birouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx birouter` — works regardless of global bin path changes after npm i -g
