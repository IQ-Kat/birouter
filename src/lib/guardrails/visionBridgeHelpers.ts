 ...
 *      Birouter itself, OpenRouter, a Gemini-OpenAI-compat URL).
 ...
 *      Birouter's own `/v1` so the vision model can be any provider
 ...
 *                that require Birouter self-loop routing.
 */
...
async function callVisionModelSingle(
  ...
  try {
    ...
    if (isAnthropic) {
      ...
      // Birouter itself or any other OpenAI-compatible endpoint instead of
      // hardcoded api.openai.com.
      const baseUrl = resolveVisionBridgeBaseUrl(config.model);

      // When routing through the Birouter self-loop (non-standard provider),
      // keep the full provider-prefixed model ID so Birouter can resolve the
      ...
      // When routing through Birouter's own API, omit the vision-bridge
      // guardrail on the sub-request to prevent infinite recursion.
      // Use sk_birouter as fallback for self-loop if no API key is resolved.
      const selfLoopApiKey = resolvedApiKey || "sk_birouter";
      const headers: Record<string, string> = {
        "Content-Type": "application/json",
        Authorization: `Bearer ${selfLoopApiKey}`,
      };
      if (useFullModelId) {
        headers["x-birouter-disabled-guardrails"] = "vision-bridge";
      }
