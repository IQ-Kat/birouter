...
async function fetchCodexSaturation(connectionId: string, dim: DimensionSpec): Promise<number> {
  ...
  const mod = await import("@birouter/open-sse/services/codexQuotaFetcher");
...
async function fetchBailianSaturation(connectionId: string, dim: DimensionSpec): Promise<number> {
  const mod = await import("@birouter/open-sse/services/bailianQuotaFetcher");
...
async function defaultAnthropicDeps(): Promise<AnthropicSaturationDeps> {
  const [providersMod, usageMod] = await Promise.all([
    ...
    import("@birouter/open-sse/services/usage"),
  ]);
  return {
...
async function defaultGenericUsageFetch(connectionId: string, provider: string): Promise<unknown> {
  const mod = await import("@birouter/open-sse/services/usage");
