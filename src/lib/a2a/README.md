...
> **Agent-to-Agent Protocol v0.3** — Enables any AI agent to use Birouter as an intelligent routing agent via JSON-RPC 2.0.

The A2A Server exposes Birouter as a **first-class agent** that other agents can discover, delegate tasks to, and collaborate with using the [A2A Protocol](https://google.github.io/A2A/).
  ...
  "skills": [
    {
      "id": "smart-routing",
      "name": "Smart Routing",
      "description": "Routes prompts through Birouter intelligent pipeline",
...
Routes prompts through Birouter's intelligent pipeline with full observability.
...
Discovers Birouter agent, sends a task, and processes the result.
"""
import requests
import json
...
```python
"""
LangChain integration — Use Birouter A2A as a custom LLM.
"""
...
class BirouterA2A(BaseLLM):
    base_url: str = "http://localhost:2004"
    api_key: str = ""
    model: str = "auto"
...
An orchestrator agent delegates code generation to Birouter, then passes the output to a review agent.

```python
def coding_pipeline(task: str):
    # Step 1: Generate code via Birouter A2A
    ...
    # Step 2: Review the code via Birouter A2A (different model)
...
Multiple agents share quota through Birouter, using the quota skill to coordinate.
...
```
Authorization: Bearer YOUR_BIROUTER_API_KEY
```

If no API key is configured on the server (`BIROUTER_API_KEY` is empty), authentication is bypassed.
