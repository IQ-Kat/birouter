 ...
 * can be iframed inside the Birouter dashboard without CORS issues.
 *
 * Security:
 ...
 *     to prevent credential leakage between Birouter and the embedded service.
 ...
 * Prevents Birouter session cookies and Authorization from leaking upstream.
 */
 ...
 *   Birouter origin, which would conflict with session management.
 * - content-security-policy / content-security-policy-report-only: the
 *   embedded service's CSP is irrelevant inside the Birouter iframe.
 * - x-frame-options: would block the iframe entirely if set to DENY/SAMEORIGIN
 *   by the embedded service (Birouter controls framing via its own CSP).
