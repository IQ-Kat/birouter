...
function buildApiBody(skill: AgentSkill, sources: BuildSources): string {
  ...
  if (ops.length === 0) {
    ...
    for (const op of ops) {
      ...
      lines.push("```bash");
      lines.push(`curl ${curlMethod}https://localhost:2004${op.path} \\`);
      lines.push('  -H "Authorization: Bearer $BIROUTER_TOKEN"');
...
function buildCliBody(skill: AgentSkill, sources: BuildSources): string {
  ...
  lines.push("```bash");
  lines.push("npm install -g birouter   # or: npx birouter");
  lines.push("birouter --version");
  lines.push("```");
  lines.push("");
