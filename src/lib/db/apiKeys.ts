...
import { resolveModelAlias } from "@birouter/open-sse/services/modelDeprecation.ts";
...
function isRedisAuthCacheEnabled(): boolean {
  return (
    process.env.BIROUTER_DISABLE_REDIS_AUTH_CACHE !== "1" &&
...
/**
 * Select an API key for internal Birouter operations (combo health checks,
...
export async function getApiKeyMetadata(
  ...
  if (isConfiguredEnvApiKey(key)) {
    ...
    // The deployment-time env key (`BIROUTER_API_KEY` / `ROUTER_API_KEY`)
