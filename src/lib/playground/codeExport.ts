...
/**
 * Endpoint suportado pelo Playground Studio. Reflete os 13 endpoints da API Birouter
...
export interface PlaygroundState {
  ...
  baseUrl: string; // ex.: "http://localhost:2004"
...
export const API_KEY_PLACEHOLDER = "$BIROUTER_API_KEY";
...
function buildCurlSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    `# Set your API key: export BIROUTER_API_KEY="your-key-here"`,
    `curl -s -X POST \\`,
    `  "${url}" \\`,
...
function buildPythonSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    ...
    `import requests`,
    ``,
    `api_key = os.environ["BIROUTER_API_KEY"]`,
    ``,
    `url = "${url}"`,
    `headers = {`,
