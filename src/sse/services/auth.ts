...
import { getQuotaScopeLabelForProvider } from "@birouter/open-sse/services/antigravityQuotaFamily.ts";
import {
...
import { isLocalProvider } from "@birouter/open-sse/config/providerRegistry.ts";
import { COOLDOWN_MS, RateLimitReason } from "@birouter/open-sse/config/constants.ts";
import {
  preflightQuota,
  isQuotaPreflightEnabled,
} from "@birouter/open-sse/services/quotaPreflight.ts";
import { resolveResilienceSettings } from "@/lib/resilience/settings";
import { resolveModelLockoutSettings } from "@/lib/resilience/modelLockoutSettings";
import { syncHealthFromDB, type KeyHealth } from "@birouter/open-sse/services/apiKeyRotator.ts";
import {
  classifyProviderError,
  PROVIDER_ERROR_TYPES,
} from "@birouter/open-sse/services/errorClassifier.ts";
...
} from "@birouter/open-sse/config/codexQuotaScopes.ts";
import {
  getProviderById,
  getProviderAlias,
  resolveProviderId,
...
export function extractSessionAffinityKey(
  ...
  const headerKey = normalizeSessionKey(
    readHeaderValue(headers, "x-codex-session-id") ??
      ...
      readHeaderValue(headers, "x-birouter-session"),
    "header"
  );
 ...
 * - `Authorization: Bearer <key>` (OpenAI / Birouter / Codex CLI / Bearer clients)
 ...
 * rejected by per-route gates that compare against Birouter keys.
 *
 ...
 * Feature #1350: Supports BIROUTER_API_KEY / ROUTER_API_KEY env vars as
...
export async function isValidApiKey(apiKey: string) {
  ...
  const envKey = process.env.BIROUTER_API_KEY || process.env.ROUTER_API_KEY;
