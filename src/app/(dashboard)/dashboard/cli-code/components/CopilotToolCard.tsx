 ...
 *
 * Feature request: https://github.com/IQ-Kat/birouter/issues/142
 */
export default function CopilotToolCard({
  ...
  const [selectedModels, setSelectedModels] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem("birouter-copilot-selected-models");
  ...
  const [selectedApiKeyId, setSelectedApiKeyId] = useState(() => {
    if (typeof window !== "undefined") {
      const savedKey = localStorage.getItem("birouter-cli-key-copilot");
  ...
  useEffect(() => {
    if (selectedModels.size > 0) {
      localStorage.setItem("birouter-copilot-selected-models", JSON.stringify([...selectedModels]));
    }
  }, [selectedModels]);
  ...
  const generateConfig = () => {
    ...
    const config = {
      name: "Birouter",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.birouter}`,
      models,
    };

    const responsesConfig = {
      name: "Birouter-responses",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.birouter}`,
  ...
  const handleApiKeyChange = (value: string) => {
    ...
    if (value) localStorage.setItem("birouter-cli-key-copilot", value);
  };
