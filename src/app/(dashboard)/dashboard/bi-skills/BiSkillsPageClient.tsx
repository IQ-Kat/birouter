...
import { BiSkillsList } from "./components/BiSkillsList";
import { BiExecutionsTab } from "./components/BiExecutionsTab";
import { BiSandboxTab } from "./components/BiSandboxTab";
import { BiMarketplaceTab } from "./components/BiMarketplaceTab";
import type { BiSkill } from "./components/BiSkillCard";
...
export function BiSkillsPageClient(): JSX.Element {
  const [skills, setSkills] = useState<BiSkill[]>([]);
  ...
  const fetchSkills = async (page: number) => {
    ...
    setSkills((res as { data?: BiSkill[] }).data || []);
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([skillsData, executionsData, settingsData]) => {
        setSkills((skillsData as { data?: BiSkill[] }).data || []);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {activeTab === "sandbox" && <BiSandboxTab />}

      {activeTab === "marketplace" && (
        <BiMarketplaceTab skillsProvider={skillsProvider} onRefreshSkills={refreshSkills} />
      )}
...
export default BiSkillsPageClient;
