...
import { resolveNestedComboTargets } from "@birouter/open-sse/services/combo.ts";
    ...
    try {
      res = await fetch(internalUrl, {
        ...
        headers: {
          ...
          // Birouter's semantic cache or other request reuse layers.
          "X-Birouter-No-Cache": "true",
          ...(target.connectionId ? { "X-Birouter-Connection": target.connectionId } : {}),
