...
// POST — write Birouter settings to Smelt config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge Birouter settings (smelt uses OpenAI-compatible config)
...
// DELETE — remove Birouter settings from Smelt config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove Birouter-managed fields
    ...
    return NextResponse.json({ success: true, message: "Smelt Birouter settings removed" });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
}
