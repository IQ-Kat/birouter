...
// Check if Birouter OpenAI-compatible provider is configured
const hasBirouterConfig = (auth) => {
  if (!auth) return false;
  const routerEntry = auth["openai-compatible"] || auth["birouter"];
  if (!routerEntry) return false;
  const baseUrl = routerEntry.baseUrl || routerEntry.baseURL || "";
  return (
    baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("birouter")
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasBirouter: hasBirouterConfig(auth),
      authPath: AUTH_PATH,
    });
  } catch (error) {
...
// POST - Configure Kilo Code to use Birouter as OpenAI-compatible provider
export async function POST(request) {
  ...
  try {
    ...
    // Add/update Birouter as openai-compatible provider
...
// DELETE - Remove Birouter config from Kilo
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove Birouter provider
    delete auth["openai-compatible"];
    delete auth["birouter"];
    ...
    return NextResponse.json({
      success: true,
      message: "Birouter settings removed from Kilo Code",
    });
  } catch (error) {
