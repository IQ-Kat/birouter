...
const hasBirouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Birouter");
};

const getBirouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Birouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getBirouterEntry(config);
...
// POST - Apply Birouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Birouter entry or append
    const idx = config.findIndex((e) => e.name === "Birouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Birouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Birouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Birouter removed from Copilot config",
    });
  } catch (error) {
