...
const parseToml = (content: string) => {
  ...
  content.split("\n").forEach((line) => {
    ...
    // Section header like [model_providers.birouter]
...
// Check if config has Birouter settings
const hasBirouterConfig = (config: string | null) => {
  if (!config) return false;
  return (
    config.includes("openai_base_url") ||
    config.includes('model_provider = "birouter"') ||
    config.includes("[model_providers.birouter]")
  );
};
...
// POST - Update Birouter settings (merge with existing config)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Update only Birouter related fields (api_key goes to auth.json, not config.toml)
    ...
    // Always create a custom provider to reliably pass wire_api and use BIROUTER_API_KEY
    parsed._root.model_provider = "birouter";
    parsed._sections["model_providers.birouter"] = {
      name: "Birouter",
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove Birouter related root fields
    delete parsed._root.openai_base_url;

    if (parsed._root.model_provider === "birouter") {
      delete parsed._root.model;
      delete parsed._root.model_provider;
    }

    // Remove birouter provider section
    delete parsed._sections["model_providers.birouter"];
