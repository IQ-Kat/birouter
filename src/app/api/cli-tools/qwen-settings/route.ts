...
const hasBirouterConfig = (settings: any) => {
  ...
  return openai.some((p: any) => {
    if (p.name?.includes("Birouter") || p.id === "birouter") return true;
...
// POST - Write Birouter config to settings.json + .env
export async function POST(request: Request) {
  ...
  try {
    ...
    const resolvedApiKey = apiKey || "sk_birouter";
    ...
    const envLines = envContent.split("\n").filter((line) => {
      // Remove old Birouter-related keys we're about to write
      return (
    ...
    // openai provider — primary, supports all models via Birouter
    const openaiEntry = {
      id: resolvedModel,
      name: `${resolvedModel} (Birouter)`,
    ...
    const openaiIdx = openaiProviders.findIndex(
      (p: any) => p && (p.baseUrl === normalizedBaseUrl || p.id === "birouter")
    );
    if (openaiIdx >= 0) {
    ...
    // anthropic provider — for Claude models via Birouter
    const anthropicEntry = {
      id: "claude-sonnet-4-6",
      name: "Claude Sonnet 4.6 (Birouter)",
    ...
    // gemini provider — for Gemini models via Birouter
    const geminiEntry = {
      id: "gemini-3-flash",
      name: "Gemini 3 Flash (Birouter)",
...
// DELETE - Remove Birouter config from settings.json and .env
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove Birouter entries from each provider type
    ...
    for (const type of providerTypes) {
      if (Array.isArray(existingConfig.modelProviders?.[type])) {
        existingConfig.modelProviders[type] = existingConfig.modelProviders[type].filter(
          (p: any) => !p.name?.includes("Birouter") && p.id !== "birouter"
        );
    ...
    return NextResponse.json({
      success: true,
      message: "Birouter settings removed from Qwen Code",
    });
  } catch (error) {
