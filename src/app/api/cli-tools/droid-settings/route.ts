...
// Check if settings has Birouter customModels.
// Multi-model entries are stored as `custom:Birouter-0`, `custom:Birouter-1`, …
// (Ported from upstream PR decolua/9router#618.)
const hasBirouterConfig = (settings: any) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(isBirouterCustomModel);
};
...
// POST - Update Birouter customModels (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Remove every existing Birouter config (multi-model: index 0..N)
    settings.customModels = settings.customModels.filter((m) => !isBirouterCustomModel(m));

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Build and prepend Birouter entries (one per requested model)
...
// DELETE - Remove Birouter customModels only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove Birouter customModels (every index, multi-model)
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter((m) => !isBirouterCustomModel(m));
