...
const hasBirouterConfig = (settings) => {
  ...
  return settings.customModels.some(m => m.id?.startsWith("custom:Birouter"));
};
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Birouter configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Birouter"));
