...
// Check if settings has Birouter config
const hasBirouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["birouter"];
};

// Read per-agent models.json and return current model id (without "birouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["birouter"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasBirouter: hasBirouterConfig(settings),
...
// POST - Update Birouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `birouter/${model}`;

    // Remove all old birouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("birouter/"))
    ...
    // Add fresh birouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`birouter/${m}`] = {};
    });

    // Remove old birouter model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("birouter/")) {
    ...
    // Update models.providers.birouter with all models
    settings.models.providers["birouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `birouter/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove Birouter settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Birouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["birouter"];
    ...
    // Remove birouter models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("birouter/"));
    ...
    // Reset agents.defaults.model.primary if it uses birouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("birouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "Birouter settings removed successfully",
    });
  } catch (error) {
