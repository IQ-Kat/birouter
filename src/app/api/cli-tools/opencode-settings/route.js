...
const hasBirouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["birouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["birouter"];
    ...
    return NextResponse.json({
      ...
      configPath: getConfigPath(),
        opencode: {
          ...
          activeModel: config?.model?.startsWith("birouter/") ? config.model.replace(/^birouter\//, "") : null,
...
// POST - Apply Birouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_birouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing birouter provider entry and its models
    const existingProvider = config.provider["birouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["birouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `birouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `birouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("birouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Birouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["birouter"]?.models) {
      delete config.provider["birouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["birouter"].models).length === 0) {
        delete config.provider["birouter"];
        if (config.model?.startsWith("birouter/")) delete config.model;
      } else if (config.model === `birouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["birouter"].models);
        config.model = `birouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire birouter provider
      if (config.provider) delete config.provider["birouter"];
      if (config.model?.startsWith("birouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("birouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Birouter settings removed from OpenCode",
    });
  } catch (error) {
