...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-birouter.env");
};
...
const hasBirouterConfig = (config) => {
  ...
  if (providers["birouter"]) return true;

  for (const [name, provider] of Object.entries(providers)) {
    if (provider.base_url && provider.base_url.includes("localhost:2004")) {
      return true;
    }
  }
...
export async function GET() {
  ...
  const hasBirouter = hasBirouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_BIROUTER_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile birouter",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["birouter"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_BIROUTER_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "birouter configuration removed from jcode",
    });
  } catch (error) {
