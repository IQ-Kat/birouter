...
import { runWithProxyContextOrDirect } from "@birouter/open-sse/utils/proxyFetch.ts";
import {
  jsonObjectSchema,
...
function resolvePublicBaseUrl(request: Request): string {
  const env = process.env.NEXT_PUBLIC_BASE_URL || process.env.BIROUTER_PUBLIC_BASE_URL;
