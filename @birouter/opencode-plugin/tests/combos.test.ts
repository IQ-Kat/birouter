 ...
 *
 * Covers:
 *   - `defaultBirouterCombosFetcher(baseURL, apiKey, timeoutMs?)`
 ...
 *   - `createBirouterProviderHook(opts, deps)` extension
...
const MODEL_PRIMARY: BirouterRawModelEntry = {
  id: "claude-primary",
  capabilities: {
    tool_calling: true,
    reasoning: true,
...
const MODEL_SECONDARY: BirouterRawModelEntry = {
  id: "claude-secondary",
  capabilities: {
    tool_calling: true,
    reasoning: false,
...
const MODEL_NO_TOOLS: BirouterRawModelEntry = {
  id: "gemini-3-flash",
...
): BirouterModelsFetcher & { callCount: () => number } {
  let n = 0;
...
function failingCombosFetcher(
  err = new Error("boom")
): BirouterCombosFetcher & { callCount: () => number } {
  let n = 0;
...
// ────────────────────────────────────────────────────────────────────────────
// defaultBirouterCombosFetcher — envelope tolerance + error surfacing
// ────────────────────────────────────────────────────────────────────────────

test("defaultBirouterCombosFetcher: parses {combos:[…]} envelope", async () => {
  ...
  try {
    const combos = await defaultBirouterCombosFetcher("https://or.example.com", "sk-test");
...
test("defaultBirouterCombosFetcher: parses bare array envelope", async () => {
  const originalFetch = globalThis.fetch;
  ...
  try {
    const combos = await defaultBirouterCombosFetcher("https://or.example.com/v1", "sk-test");
...
test("defaultBirouterCombosFetcher: strips trailing /v1 before /api/combos", async () => {
  const originalFetch = globalThis.fetch;
  let observedUrl = "";
  globalThis.fetch = (async (input: unknown) => {
  ...
  try {
    await defaultBirouterCombosFetcher("https://or.example.com/v1/", "sk-test");
...
test("defaultBirouterCombosFetcher: throws on non-2xx with status code in message", async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = (async () => {
  ...
  try {
    await assert.rejects(
      async () => {
        await defaultBirouterCombosFetcher("https://or.example.com", "sk-bad");
      },
      (err: unknown) => {
...
test("defaultBirouterCombosFetcher: throws when apiKey missing", async () => {
  await assert.rejects(
    async () => defaultBirouterCombosFetcher("https://or.example.com", ""),
    /apiKey required/
  );
});

test("defaultBirouterCombosFetcher: throws when baseURL missing", async () => {
  await assert.rejects(async () => defaultBirouterCombosFetcher("", "sk-test"), /baseURL required/);
});
...
test("mapComboToModelV2: api block matches providerId + baseURL", () => {
  ...
  assert.equal(m.providerID, "birouter-preprod");
...
test("mapComboToModelV2: explicit member temperature=false drops combo temperature=false", () => {
  const tempFalse: BirouterRawModelEntry = {
    id: "no-temp",
...
// ────────────────────────────────────────────────────────────────────────────
// createBirouterProviderHook — combos merge + collision + soft-fail + cache
// ────────────────────────────────────────────────────────────────────────────
...
test("models() returns combo entries merged into the map", async () => {
  ...
  assert.ok(out["opencode-birouter/claude-primary"]);
  assert.ok(out["opencode-birouter/claude-secondary"]);
  assert.ok(out["opencode-birouter/gemini-3-flash"]);
  assert.ok(out["opencode-birouter/claude-tier"]);

  const combo = out["opencode-birouter/claude-tier"];
  assert.equal(combo.name, "Claude Tier");
  assert.equal(combo.providerID, "opencode-birouter");
...
test("models(): combo with unknown member ids degrades to all-false LCD posture", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  assert.ok(out["opencode-birouter/phantom-combo"]);
  // With zero resolvable members, LCD = all-false (defensive posture).
  assert.equal(out["opencode-birouter/phantom-combo"].capabilities.toolcall, false);
  assert.equal(out["opencode-birouter/phantom-combo"].capabilities.reasoning, false);
  assert.equal(out["opencode-birouter/phantom-combo"].limit.context, 0);
});
...
test("models(): hidden combos are excluded from the map", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  assert.ok(out["opencode-birouter/visible"]);
  assert.ok(!out["opencode-birouter/hidden"], "hidden combo must be omitted");
});
...
test("models(): combo name exactly matches raw model id → raw deleted, raw deleted, no warn", async () => {
  ...
  // the real Birouter payload where /v1/models pre-mirrors combos as
  // no-slash raw entries whose ids match /api/combos friendly names.
  const colliderCombo: BirouterRawCombo = {
    id: "uuid-collider",
  ...
  assert.ok(out["opencode-birouter/claude-primary"], "combo surfaces under prefixed key");
  assert.equal(out["opencode-birouter/claude-primary"].name, "claude-primary");
...
test("models(): two combos with same slug → second gets disambiguator suffix", async () => {
  ...
  assert.ok(out["opencode-birouter/claude"], "first combo at prefixed slug");
  assert.ok(out["opencode-birouter/claude-uuid"], "second combo disambiguated by id prefix");
});
...
test("models(): combos cached + reused within TTL (one combo fetch per TTL window)", async () => {
  ...
  assert.ok(second["opencode-birouter/claude-tier"]);
});
...
test("models(): nested combo-ref context is the min of nested + raw members", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  const masterLight = out["opencode-birouter/master-light"];
