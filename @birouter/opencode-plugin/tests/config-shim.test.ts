 ...
 *
 * Covers `createBirouterConfigHook(opts, deps)`:
 *   - happy path: valid auth.json → mutates input.provider[id] with the
 *     stripped per-model shape (mirroring `@birouter/opencode-provider`).
...
import {
  ...
  createBirouterConfigHook,
  createBirouterProviderHook,
  BirouterPlugin,
  resolveBirouterPluginOptions,
  type BirouterCombosFetcher,
  type BirouterEnrichmentEntry,
  type BirouterEnrichmentFetcher,
  type BirouterEnrichmentMap,
  type BirouterFetchCache,
  type BirouterModelsFetcher,
  type BirouterProviderConnection,
  type BirouterProvidersFetcher,
  type BirouterRawCombo,
  type BirouterRawModelEntry,
  type BirouterReadAuthJson,
  type BirouterStaticProviderEntry,
} from "../src/index.js";
...
const MODEL_CLAUDE: BirouterRawModelEntry = {
...
const MODEL_GEMINI: BirouterRawModelEntry = {
  id: "gemini-3-flash",
...
const COMBO_CLAUDE_TIER: BirouterRawCombo = {
...
): BirouterReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: BirouterReadAuthJson = async () => {
    n++;
...
function throwingReadAuthJson(): BirouterReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: BirouterReadAuthJson = async () => {
    n++;
...
function stubModelsFetcher(
  payload: BirouterRawModelEntry[]
): BirouterModelsFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: BirouterModelsFetcher = async (baseURL, apiKey) => {
    n++;
...
function stubCombosFetcher(
  payload: BirouterRawCombo[]
): BirouterCombosFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: BirouterCombosFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingModelsFetcher(): BirouterModelsFetcher & { callCount: () => number } {
  let n = 0;
  const f: BirouterModelsFetcher = async () => {
    n++;
...
function throwingCombosFetcher(): BirouterCombosFetcher & { callCount: () => number } {
  let n = 0;
  const f: BirouterCombosFetcher = async () => {
    n++;
...
function stubEnrichmentFetcher(payload: BirouterEnrichmentMap): BirouterEnrichmentFetcher & {
  callCount: () => number;
  callsBy: () => Array<[string, string]>;
} {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: BirouterEnrichmentFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingEnrichmentFetcher(): BirouterEnrichmentFetcher & { callCount: () => number } {
  let n = 0;
  const f: BirouterEnrichmentFetcher = async () => {
    n++;
...
test("config: with valid auth.json + apiKey + baseURL → mutates input.provider[id] with stripped models block", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-birouter": { type: "api", key: "sk-test-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createBirouterConfigHook(
    { providerId: "birouter" },
  ...
  const provider = (input as { provider: Record<string, BirouterStaticProviderEntry> }).provider;
  const entry = provider["opencode-birouter"];
  assert.ok(entry, "input.provider['opencode-birouter'] set");
  assert.equal(entry.npm, "@ai-sdk/openai-compatible");
  assert.equal(entry.name, "Birouter");
  ...
  const claude = entry.models["opencode-birouter/claude-sonnet-4-6"];
  ...
  const combo = entry.models["opencode-birouter/claude-tier"];
...
//     prefixed (`opencode-birouter`). Without the fallback the lookup misses
...
test("config: auth.json under bare key (pre-prefix login) resolves via dual-key fallback", async () => {
  // Stored under bare `birouter` (the key OC wrote before the auto-prefix fix),
  // but the resolved providerId is now `opencode-birouter`.
  const readAuthJson = stubReadAuthJson({
    birouter: { type: "api", key: "sk-bare-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createBirouterConfigHook(
    { providerId: "birouter" }, // resolves to opencode-birouter internally
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: prefixed key wins over bare key when both present (dual-key precedence)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-birouter": { type: "api", key: "sk-prefixed", baseURL: "https://pref.example/v1" },
    birouter: { type: "api", key: "sk-bare", baseURL: "https://bare.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const entry = (input as { provider: Record<string, BirouterStaticProviderEntry> }).provider[
    "opencode-birouter"
  ];
  assert.ok(entry);
  assert.equal(
...
test("config: existing input.provider[id] → no overwrite (respect manual override)", async () => {
  const manual = {
    ...
    name: "Manual Birouter",
    options: { baseURL: "http://manual/v1", apiKey: "manual-key" },
    models: { "manual-model": { name: "manual-model" } },
  };
  const readAuthJson = stubReadAuthJson({
    "opencode-birouter": { type: "api", key: "sk-test", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const input = makeInput({ "opencode-birouter": manual });
  await hook(input);

  const provider = (input as { provider: Record<string, unknown> }).provider;
  assert.equal(provider["opencode-birouter"], manual, "manual override preserved by reference");
...
test("config: fetchers throw → warn + emit stub entry with models: {}", async () => {
  ...
  // test below).
  const hook = createBirouterConfigHook(
    { providerId: "birouter", features: { diskCache: false } },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: combos fetcher throws → emit models-only catalog (no combos in models block)", async () => {
  ...
  assert.deepEqual(ids, [
    "opencode-birouter/claude-sonnet-4-6",
    "opencode-birouter/gemini-3-flash",
  ]);
  assert.equal(entry.models["opencode-birouter/claude-tier"], undefined, "no combo entry");
  assert.ok(
    logger.entries.some((e) => String(e[0]).includes("/api/combos fetch failed")),
    "combos-fetch breadcrumb emitted"
  );
...
test("config: baseURL from auth.json takes precedence when opts.baseURL absent", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-birouter": { type: "api", key: "sk-test", baseURL: "https://creds.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createBirouterConfigHook(
    { providerId: "birouter" }, // NO opts.baseURL
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: opts.baseURL wins over auth.json's stored baseURL", async () => {
  ...
  const hook = createBirouterConfigHook(
    { providerId: "birouter", baseURL: "https://opts.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: no baseURL resolvable (no opts, no auth.json baseURL) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-birouter": { type: "api", key: "sk-test" }, // NO baseURL on the credential
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
...
test("config: multi-instance — two plugins with different providerIds publish to their own keys without collision", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-birouter-prod": {
      type: "api",
      key: "sk-prod",
      baseURL: "https://prod.example/v1",
    },
    "opencode-birouter-preprod": {
      type: "api",
      key: "sk-preprod",
  ...
  const hookA = createBirouterConfigHook(
    { providerId: "birouter-prod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const hookB = createBirouterConfigHook(
    { providerId: "birouter-preprod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  ...
  const provider = (input as { provider: Record<string, BirouterStaticProviderEntry> }).provider;
  assert.ok(provider["opencode-birouter-prod"], "prod block present");
  assert.ok(provider["opencode-birouter-preprod"], "preprod block present");
  assert.equal(provider["opencode-birouter-prod"].options.apiKey, "sk-prod");
  assert.equal(provider["opencode-birouter-preprod"].options.apiKey, "sk-preprod");
  assert.equal(provider["opencode-birouter-prod"].options.baseURL, "https://prod.example/v1");
  assert.equal(provider["opencode-birouter-preprod"].options.baseURL, "https://preprod.example/v1");
  assert.notEqual(
    provider["opencode-birouter-prod"],
    provider["opencode-birouter-preprod"],
...
test("config + provider share cache: second call uses cached fetch result (single fetch per TTL)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-birouter": { type: "api", key: "sk-shared", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([COMBO_CLAUDE_TIER]);
  ...
  const configHook = createBirouterConfigHook(
    { providerId: "birouter", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, cache: sharedCache, logger }
  );
  const providerHook = createBirouterProviderHook(
    { providerId: "birouter", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
...
test("provider → config order also dedupes (cache populated by provider, consumed by config)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-birouter": { type: "api", key: "sk-reverse", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const sharedCache: BirouterFetchCache = new Map();
...
test("buildStaticProviderEntry: stripped per-model shape matches sibling @birouter/opencode-provider", () => {
  const resolved = resolveBirouterPluginOptions({
    providerId: "birouter",
    displayName: "Birouter",
  });
  ...
  assert.equal(block.name, "Birouter");
  ...
  const claude = block.models["opencode-birouter/claude-sonnet-4-6"];
...
test("buildStaticProviderEntry: empty fetch results → stub block with models: {}", () => {
  const resolved = resolveBirouterPluginOptions({ providerId: "birouter" });
...
test("buildStaticProviderEntry: hidden combos are excluded", () => {
  ...
  assert.equal(block.models["opencode-birouter/claude-tier"], undefined);
  assert.ok(block.models["opencode-birouter/claude-sonnet-4-6"]);
});
...
test("buildStaticProviderEntry: emits release_date when raw carries it; omits when null", () => {
  const resolved = resolveBirouterPluginOptions({ providerId: "birouter" });
  const withDate: BirouterRawModelEntry = {
    ...MODEL_CLAUDE,
    id: "claude-with-date",
  ...
  assert.equal(block.models["opencode-birouter/claude-with-date"].release_date, "2026-02-19");
  assert.equal(block.models["opencode-birouter/gemini-3-flash"].release_date, undefined);
});

test("buildStaticProviderEntry: combo modalities = intersection of members (LCD)", () => {
  const resolved = resolveBirouterPluginOptions({ providerId: "birouter" });
  const TEXT_ONLY: BirouterRawModelEntry = {
    id: "text-only",
  ...
  const combo = block.models["opencode-birouter/mixed-tier"];
...
// ────────────────────────────────────────────────────────────────────────────
// Integration: BirouterPlugin factory now exposes config hook
// ────────────────────────────────────────────────────────────────────────────

test("BirouterPlugin factory exposes config hook alongside auth + provider", async () => {
  const hooks = await BirouterPlugin({} as never, { providerId: "birouter" });
...
test("config: auth.json entry of wrong type (oauth) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-birouter": { type: "oauth", refresh: "r", access: "a", expires: 0 },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createBirouterConfigHook(
    { providerId: "birouter", baseURL: "https://or.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: initialises input.provider when undefined", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-birouter": { type: "api", key: "sk", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  assert.ok(provider!["opencode-birouter"]);
});
...
test("config: enrichment fetched + name overlaid on raw-model entries", async () => {
  ...
  const enrichmentFetcher = stubEnrichmentFetcher(
    new Map<string, BirouterEnrichmentEntry>([
  ...
  assert.ok(entry);
  assert.equal(entry.models["opencode-birouter/claude-sonnet-4-6"].name, "Claude Sonnet 4.6");
  assert.equal(entry.models["opencode-birouter/gemini-3-flash"].name, "Gemini 3 Flash");
  // Combo names still come from /api/combos — enrichment overlay does NOT touch combos.
  assert.equal(entry.models["opencode-birouter/claude-tier"].name, "Claude Tier");
...
test("config: features.enrichment=false skips enrichment fetch + keeps raw-id names", async () => {
  ...
  const combosFetcher = stubCombosFetcher([]);
  const enrichmentFetcher = stubEnrichmentFetcher(
    new Map<string, BirouterEnrichmentEntry>([["claude-sonnet-4-6", { name: "Claude Sonnet 4.6" }]])
  );
  const logger = captureWarn();

  const hook = createBirouterConfigHook(
    { providerId: "birouter", features: { enrichment: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
  ...
  assert.equal(
    entry.models["opencode-birouter/claude-sonnet-4-6"].name,
    "claude-sonnet-4-6",
    "raw id retained"
  );
});
...
function stubProvidersFetcher(payload: BirouterProviderConnection[]): BirouterProvidersFetcher & {
  callCount: () => number;
} {
  let n = 0;
  const f: BirouterProvidersFetcher = async () => {
    n++;
    return payload;
  };
  return Object.assign(f, { callCount: () => n });
...
function throwingProvidersFetcher(): BirouterProvidersFetcher & { callCount: () => number } {
  let n = 0;
  const f: BirouterProvidersFetcher = async () => {
    n++;
    throw new Error("ETIMEDOUT");
...
const MODEL_CC_OPUS: BirouterRawModelEntry = {
  id: "cc/claude-opus-4-7",
  capabilities: { tool_calling: true, reasoning: true, temperature: true },
  context_length: 200_000,
};
const MODEL_NV_LLAMA: BirouterRawModelEntry = {
...
test("config: usableOnly=true → drops models for non-usable providers, keeps usable + unknown", async () => {
  ...
  const hook = createBirouterConfigHook(
    { providerId: "birouter", baseURL: "https://or.example/v1", features: { usableOnly: true } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, providersFetcher }
  );
...
test("config: diskCache hydrates stale snapshot when /v1/models throws", async () => {
  ...
  const hook = createBirouterConfigHook(
    { providerId: "birouter", features: { diskCache: true } },
    {
      readAuthJson,
      fetcher,
      combosFetcher,
  ...
  assert.ok(
    entry.models["opencode-birouter/claude-sonnet-4-6"],
...
test("config: providerTag (default-on) prepends '<provider> - ' to enriched raw-model names", async () => {
  ...
  assert.equal(entry.models["opencode-birouter/gemini-3-flash"].name, "Gemini - Gemini 3 Flash");
...
test("config: providerTag=false suppresses the suffix", async () => {
  ...
  const hook = createBirouterConfigHook(
    { providerId: "birouter", features: { providerTag: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: providerTag falls back to UPPER(alias) when providerDisplayName missing", async () => {
  ...
  // a slot Birouter hasn't curated a human label for yet. We still
  ...
  assert.equal(entry.models["opencode-birouter/claude-sonnet-4-6"].name, "CC - Claude Sonnet 4.6");
});
...
test("config: providerTag is idempotent — second hook call doesn't double-suffix", async () => {
  ...
  const hook = createBirouterConfigHook(
    { providerId: "birouter", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, cache: sharedCache, logger }
  );

  const inputA = makeInput();
  await hook(inputA);
  const entryA = (inputA as { provider: Record<string, BirouterStaticProviderEntry> }).provider[
    "opencode-birouter"
  ];
  assert.equal(
    entryA.models["opencode-birouter/claude-sonnet-4-6"].name,
    "Claude - Claude Sonnet 4.6"
  );
  ...
  await hook(inputB);
  const entryB = (inputB as { provider: Record<string, BirouterStaticProviderEntry> }).provider[
    "opencode-birouter"
  ];
  assert.equal(
    entryB.models["opencode-birouter/claude-sonnet-4-6"].name,
    "Claude - Claude Sonnet 4.6"
  );
});
...
test("buildStaticProviderEntry: nested combo-ref context is the bottleneck across the graph", () => {
  const resolved = resolveBirouterPluginOptions({ providerId: "birouter" });
  const rawModels: BirouterRawModelEntry[] = [
    {
      id: "raw-big",
      context_length: 200_000,
      max_output_tokens: 64_000,
  ...
  const rawCombos: BirouterRawCombo[] = [
    {
      id: "tiny-combo",
      name: "TinyCombo",
  ...
  const parent = block.models["opencode-birouter/parent"];
