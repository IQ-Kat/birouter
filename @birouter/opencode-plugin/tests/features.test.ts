...
import {
  ...
  defaultBirouterEnrichmentFetcher,
  defaultBirouterCompressionMetaFetcher,
...
test("parseBirouterPluginOptions: empty features object → preserved", () => {
  const r = parseBirouterPluginOptions({ features: {} });
  assert.deepEqual(r, { features: {} });
});

test("parseBirouterPluginOptions: all boolean features set → preserved", () => {
  const r = parseBirouterPluginOptions({
    features: {
      combos: true,
      enrichment: true,
...
test("parseBirouterPluginOptions: mcpToken string → preserved", () => {
  const r = parseBirouterPluginOptions({
...
test("parseBirouterPluginOptions: unknown features key → throws (strict)", () => {
  assert.throws(
    () =>
      parseBirouterPluginOptions({
        features: { combos: true, unknown_field: "oops" },
      }),
    /Invalid @birouter\/opencode-plugin options/
  );
});

test("parseBirouterPluginOptions: non-boolean for boolean feature → throws", () => {
  assert.throws(
    () =>
      parseBirouterPluginOptions({
...
test("parseBirouterPluginOptions: empty mcpToken → throws (min 1)", () => {
  assert.throws(
    () => parseBirouterPluginOptions({ features: { mcpToken: "" } }),
    /Invalid @birouter\/opencode-plugin options/
  );
});
...
const SAMPLE_RAW: BirouterRawModelEntry[] = [
  {
    id: "claude-sonnet-4-6",
    object: "model",
    created: 0,
...
test("provider hook: enrichment fetcher called when features.enrichment !== false", async () => {
  let called = 0;
  const enrichment: BirouterEnrichmentMap = new Map([
    ["claude-sonnet-4-6", { name: "Claude Sonnet 4.6", pricing: { input: 3, output: 15 } }],
  ]);
  const hook = createBirouterProviderHook(
    { providerId: "birouter", baseURL: "https://or.example.com/v1" },
    {
  ...
  const m = out["opencode-birouter/claude-sonnet-4-6"];
...
test("provider hook: enrichment fetcher NOT called when features.enrichment:false", async () => {
  ...
  assert.equal(
    out["opencode-birouter/claude-sonnet-4-6"].name,
    "claude-sonnet-4-6",
    "raw id preserved"
  );
});
...
test("provider hook: compression metadata fetcher called when opted in", async () => {
  let called = 0;
  const compressionCombos: BirouterCompressionCombo[] = [
    {
      id: "default-caveman",
  ...
  const combo = out["opencode-birouter/claude-primary"];
...
const stubAuthJson = (apiKey: string) => async () => ({
  "opencode-birouter": { type: "api" as const, key: apiKey },
});
...
test("config hook: MCP auto-emit OFF by default (no mcp entry)", async () => {
  ...
  assert.ok(input.provider?.["opencode-birouter"], "provider block written");
...
test("config hook: features.mcpAutoEmit:true writes mcp entry with provider apiKey", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-birouter"] as
...
test("config hook: features.mcpToken overrides provider apiKey in mcp Bearer", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-birouter"] as { headers: Record<string, string> };
  assert.equal(
...
test("config hook: existing operator mcp.<providerId> wins (no overwrite)", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {
    mcp: { "opencode-birouter": { type: "custom-user-entry", url: "https://manual.example/mcp" } },
  };
  await hook(input as never);
  assert.deepEqual(
    input.mcp?.["opencode-birouter"],
...
test("config hook: features.mcpAutoEmit:true with /v1 in baseURL → strips correctly", async () => {
  const hook = createBirouterConfigHook(
    ...
    {
      readAuthJson: async () => ({
        "opencode-birouter-preprod": { type: "api" as const, key: "sk-preprod" },
      }),
      fetcher: async () => SAMPLE_RAW,
      combosFetcher: async () => [],
      logger: { warn: () => {} },
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-birouter-preprod"] as { url: string };
  assert.equal(
    entry.url,
...
test("defaultBirouterEnrichmentFetcher: empty baseURL → empty map", async () => {
  const m = await defaultBirouterEnrichmentFetcher("", "sk", 100);
  assert.equal(m.size, 0);
});

test("defaultBirouterEnrichmentFetcher: empty apiKey → empty map", async () => {
  const m = await defaultBirouterEnrichmentFetcher("https://or.example.com", "", 100);
  assert.equal(m.size, 0);
});

test("defaultBirouterCompressionMetaFetcher: empty baseURL → empty array", async () => {
  const arr = await defaultBirouterCompressionMetaFetcher("", "sk", 100);
...
test("defaultBirouterEnrichmentFetcher: merges names from /api/pricing/models and prices from /api/pricing", async () => {
  ...
  try {
    const map = await defaultBirouterEnrichmentFetcher(
...
test("defaultBirouterEnrichmentFetcher: name-only when pricing endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing/models")) {
  ...
  try {
    const map = await defaultBirouterEnrichmentFetcher("https://or.example.com", "sk-test", 5_000);
    const opus = map.get("cc/claude-opus-4-7");
...
test("defaultBirouterEnrichmentFetcher: pricing-only when catalog endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing")) {
...
): BirouterEnrichmentMap {
  const map: BirouterEnrichmentMap = new Map();
...
test("canonicalDedupSet: drops canonical row when alias twin present", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: BirouterRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as BirouterRawModelEntry,
    { id: "claude/claude-opus-4-7" } as BirouterRawModelEntry,
  ];
...
test("canonicalDedupSet: keeps standalone canonical row (no alias twin) — never hides a model", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: BirouterRawModelEntry[] = [
    { id: "claude/claude-opus-99" } as BirouterRawModelEntry, // canonical only — no `cc/claude-opus-99`
  ];
  const drop = canonicalDedupSet(raw, c2a);
...
test("canonicalDedupSet: multi-provider — drops all canonical twins where alias exists", () => {
  ...
  const raw: BirouterRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as BirouterRawModelEntry,
    { id: "claude/claude-opus-4-7" } as BirouterRawModelEntry,
    { id: "cx/gpt-5.5" } as BirouterRawModelEntry,
    { id: "codex/gpt-5.5" } as BirouterRawModelEntry,
    { id: "pol/openai-large" } as BirouterRawModelEntry,
    { id: "pollinations/openai-large" } as BirouterRawModelEntry,
  ];
  const drop = canonicalDedupSet(raw, c2a);
  assert.equal(drop.has("claude/claude-opus-4-7"), true);
