 ...
 *
 * Validates that two `BirouterPlugin(input, opts)` invocations with
 ...
 *
 *   "plugin": [
 *     ["@birouter/opencode-plugin", {"providerId": "birouter-prod",    "baseURL": "https://or.example/v1"}],
 *     ["@birouter/opencode-plugin", {"providerId": "birouter-preprod", "baseURL": "https://or-preprod.example/v1"}]
 *   ]
 *
 * Assertions:
...
import { BirouterPlugin } from "../src/index.js";

const fakeInput = {} as Parameters<typeof BirouterPlugin>[0];

test("multi-instance: two plugin invocations bind to their own providerId", async () => {
  const a = await BirouterPlugin(fakeInput, {
    providerId: "birouter-prod",
    baseURL: "https://a.example/v1",
  });
  const b = await BirouterPlugin(fakeInput, {
    providerId: "birouter-preprod",
    baseURL: "https://b.example/v1",
  });

  assert.equal(a.auth?.provider, "opencode-birouter-prod");
  assert.equal(b.auth?.provider, "opencode-birouter-preprod");
});
...
test("multi-instance: identical opts twice still yield independent objects", async () => {
  ...
  const first = await BirouterPlugin(fakeInput, { ...opts });
  const second = await BirouterPlugin(fakeInput, { ...opts });
...
test("multi-instance: invalid opts on one instance does not poison the other", async () => {
  ...
  await assert.rejects(
    () => BirouterPlugin(fakeInput, { providerId: "bad id!" } as never),
    /providerId/
  );
  const ok = await BirouterPlugin(fakeInput, {
