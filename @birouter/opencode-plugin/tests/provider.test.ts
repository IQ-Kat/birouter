 ...
 *
 * Covers `createBirouterProviderHook(opts, deps)`:
...
const FIXTURE: BirouterRawModelEntry[] = [
  {
    id: "claude-primary",
    object: "model",
    owned_by: "combo",
...
function stubFetcher(payload: BirouterRawModelEntry[]): BirouterModelsFetcher & {
  callCount: () => number;
...
test("createBirouterProviderHook: default providerId is 'birouter'", () => {
  const hook = createBirouterProviderHook(undefined, { combosFetcher: async () => [] });
  assert.equal(hook.id, "opencode-birouter");
});

test("createBirouterProviderHook: custom providerId binds to hook.id (multi-instance)", () => {
  const a = createBirouterProviderHook(
    { providerId: "birouter-preprod" },
    { combosFetcher: async () => [] }
  );
  const b = createBirouterProviderHook(
    { providerId: "birouter-local" },
    { combosFetcher: async () => [] }
  );
  assert.equal(a.id, "opencode-birouter-preprod");
  assert.equal(b.id, "opencode-birouter-local");
});
...
test("models: returns {} when no baseURL resolvable (no opts.baseURL and no auth.baseURL)", async () => {
  const fetcher = stubFetcher(FIXTURE);
  const hook = createBirouterProviderHook({}, { fetcher, combosFetcher: async () => [] });
...
test("models: maps a sample /v1/models entry to ModelV2 (sanity)", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-abc") as never });
  const claude = out["opencode-birouter/claude-primary"];
  ...
  assert.equal(claude.id, "opencode-birouter/claude-primary");
  assert.equal(claude.name, "claude-primary");
  assert.equal(claude.providerID, "opencode-birouter");
  ...
  // cost is zeroed (Birouter /v1/models has no pricing)
