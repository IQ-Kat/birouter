/**
 * OpenCode provider plugin for Birouter AI Gateway.
 *
 * Generates an OpenCode-compatible provider object that points to a running
 * Birouter instance. The output follows the OpenCode config schema
 * (https://opencode.ai/config.json) and delegates the runtime to
 * `@ai-sdk/openai-compatible` so OpenCode can drive any Birouter-exposed
 ...
 *     const provider = createBirouterProvider({ baseURL, apiKey });
 *     // provider -> the value to place under provider.birouter in opencode.json
 *     ```
 *
...
export const BIROUTER_PROVIDER_NPM = "@ai-sdk/openai-compatible" as const;
 ...
 *
 * Curated set covering the most commonly deployed Birouter models. Synced
 * with the Alph4d0g/opencode-birouter-auth BIROUTER_DEFAULT_MODELS constant
 * (https://github.com/Alph4d0g/opencode-birouter-auth, MIT) and extended
 ...
 *
 * Mirrors the capability shape used by Alph4d0g/opencode-birouter-auth
 * (https://github.com/Alph4d0g/opencode-birouter-auth, MIT).
 */
 ...
 * Matches the context lengths used by Birouter's provider registry.
 */
export const BIROUTER_DEFAULT_MODEL_CONTEXT_LENGTHS: Record<string, number> = {
 ...
 * model via `BirouterProviderOptions.modelCapabilities`.
 */
export const BIROUTER_DEFAULT_MODEL_CAPABILITIES: Record<string, ModelCapabilities> = {
...
export interface BirouterProviderOptions {
  /** Birouter base URL, with or without trailing `/v1`. Required. */
  baseURL: string;
  /** Birouter API key. Required. Use `sk_birouter` for local instances without REQUIRE_API_KEY. */
  apiKey: string;
  /** Override the display name shown in OpenCode. Default: `"Birouter"`. */
  displayName?: string;
   ...
   * `BIROUTER_DEFAULT_MODEL_CAPABILITIES` for ids in the default catalog;
   * for custom ids the override is used verbatim.
   */
  modelCapabilities?: Record<string, ModelCapabilities>;
  /**
   * Optional per-model context-length overrides (tokens). Takes precedence
   * over the static `BIROUTER_DEFAULT_MODEL_CONTEXT_LENGTHS` map but is
   ...
   * Emitted as `"birouter/<id>"`. When omitted the key is not written.
   */
  model?: string;
  /**
...
/** Per-model entry written under `provider.birouter.models[id]`. */
...
export interface OpenCodeProviderEntry {
  /** Identifier of the OpenCode runtime package that will speak to Birouter. */
  npm: typeof BIROUTER_PROVIDER_NPM;
...
export interface OpenCodeConfigDocument {
  ...
  /** Primary model for OpenCode, e.g. `"birouter/claude-sonnet-4-5-thinking"`. */
  model?: string;
  /** Secondary / cheap model for OpenCode, e.g. `"birouter/gemini-3-flash"`. */
  small_model?: string;
  provider: {
    [BIROUTER_PROVIDER_KEY]: OpenCodeProviderEntry;
  };
}

function requireNonEmpty(value: unknown, field: string): string {
  if (typeof value !== "string") {
    throw new TypeError(`@birouter/opencode-provider: ${field} must be a string`);
  }
  const trimmed = value.trim();
  if (!trimmed) {
    throw new Error(`@birouter/opencode-provider: ${field} is required and cannot be empty`);
  }
  return trimmed;
}
...
export function normalizeBaseURL(rawBaseURL: string): string {
  ...
  try {
    ...
    throw new Error(
      `@birouter/opencode-provider: baseURL is not a valid URL: ${JSON.stringify(rawBaseURL)}`
    );
  }
  let base = trimmed;
  let end = base.length;
...
/**
 * Build the `provider.birouter` entry for an OpenCode config document.
 * The returned object is JSON-serialisable and safe to embed verbatim.
 */
export function createBirouterProvider(options: BirouterProviderOptions): OpenCodeProviderEntry {
  ...
  for (const raw of modelList) {
    ...
    seen.add(id);
    const defaults = BIROUTER_DEFAULT_MODEL_CAPABILITIES[id] ?? {};
    ...
    const rawContextLength =
      ...
      BIROUTER_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    const contextLength =
  ...
  return {
    npm: BIROUTER_PROVIDER_NPM,
    name: options.displayName?.trim() || "Birouter",
...
/**
 * Build a full OpenCode config document (with `$schema` + `provider.birouter`).
 * Useful when scaffolding a fresh `opencode.json`.
 *
 * When `options.model` / `options.smallModel` are supplied they are emitted as
 * top-level `model` / `small_model` keys prefixed with `"birouter/"` so
 * OpenCode resolves them through the configured provider.
 */
export function buildBirouterOpenCodeConfig(
  options: BirouterProviderOptions
): OpenCodeConfigDocument {
  const doc: OpenCodeConfigDocument = {
    $schema: OPENCODE_CONFIG_SCHEMA,
    provider: {
      [BIROUTER_PROVIDER_KEY]: createBirouterProvider(options),
    },
  };

  if (options.model !== undefined) {
    const id = options.model.trim();
    if (id) doc.model = `${BIROUTER_PROVIDER_KEY}/${id}`;
  }

  if (options.smallModel !== undefined) {
    const id = options.smallModel.trim();
    if (id) doc.small_model = `${BIROUTER_PROVIDER_KEY}/${id}`;
  }
...
/**
 * Merge the Birouter provider entry (and optional `model` / `small_model`
 ...
 * present are not removed. If `existing.provider.birouter` already exists it
...
export function mergeIntoExistingConfig(
  ...
  const partial = buildBirouterOpenCodeConfig(options);
  ...
  merged.provider = {
    ...existingProvider,
    [BIROUTER_PROVIDER_KEY]: partial.provider[BIROUTER_PROVIDER_KEY],
  };
 ...
 */
export const BIROUTER_MCP_DEFAULT_SCOPES = [
  "read:health",
  "read:combos",
  "read:quota",
  "read:usage",
...
export type BirouterMCPScope = (typeof BIROUTER_MCP_DEFAULT_SCOPES)[number] | string;

export interface BirouterMCPOptions {
  /** Absolute path to the MCP server entry point (TypeScript or compiled JS). */
  serverPath: string;
  /** Birouter API key forwarded to the MCP server as `BIROUTER_API_KEY`. */
  apiKey: string;
  /**
   * Management API key used for management-scoped operations.
   * When supplied it is forwarded as `BIROUTER_MANAGEMENT_API_KEY`.
   */
  managementApiKey?: string;
  /**
   * Comma-separated scope list passed as `BIROUTER_MCP_SCOPES`.
   * When omitted `BIROUTER_MCP_ENFORCE_SCOPES` is not set and all scopes are
   * available (development default). Pass an explicit list to restrict access.
   */
  scopes?: BirouterMCPScope[];
  /**
...
/**
 * Build the `mcp.servers.birouter` entry for an OpenCode config document.
 *
 * @example
 * ```ts
 * const mcpEntry = createBirouterMCPEntry({
 *   serverPath: "/home/user/.local/share/birouter/open-sse/mcp-server/server.ts",
 *   apiKey: "sk_birouter",
 *   managementApiKey: "sk_manage_...",
 *   scopes: ["read:health", "read:combos", "execute:completions"],
 * });
 * // Place at config.mcp.servers.birouter
 * ```
 */
export function createBirouterMCPEntry(options: BirouterMCPOptions): OpenCodeMCPServerEntry {
  ...
  if (options.managementApiKey !== undefined) {
    ...
    if (mgmtKey) env.BIROUTER_MANAGEMENT_API_KEY = mgmtKey;
  }

  if (options.scopes !== undefined && options.scopes.length > 0) {
    env.BIROUTER_MCP_ENFORCE_SCOPES = "true";
    env.BIROUTER_MCP_SCOPES = options.scopes.join(",");
  }
...
async function fetchJSON<T>(url: string, apiKey: string, timeoutMs: number): Promise<T> {
  ...
  try {
    ...
    throw new Error(`@birouter/opencode-provider: request to ${url} failed: ${message}`);
  } finally {
    clearTimeout(timer);
  }
}
 ...
 * The shape mirrors the subset of fields that Birouter's `/v1/models`
 * endpoint reliably provides across versions, normalised from both
 * camelCase and snake_case variants used by different Birouter releases.
 *
 * Attribution: field-variant normalisation logic adapted from
 * https://github.com/Alph4d0g/opencode-birouter-auth (MIT).
 */
export interface BirouterLiveModel {
  id: string;
  name: string;
...
/**
 * Fetch the live model catalog from a running Birouter instance.
 *
 ...
 * `display_name`) field variants across Birouter versions.
 *
 * Useful for dynamically populating the `models` option of
 * `createBirouterProvider` / `buildBirouterOpenCodeConfig` instead of
 * relying on `BIROUTER_DEFAULT_OPENCODE_MODELS`.
 *
 * @param baseURL   - Birouter base URL (with or without `/v1`).
 * @param apiKey    - Birouter API key.
...
export async function fetchLiveModels(
  ...
  const models: BirouterLiveModel[] = [];
  for (const raw of rawList) {
    ...
    // Extract context_length from Birouter's /v1/models response.
    // Birouter returns context_length in snake_case for both synced
 ...
 */
export type BirouterCompressionOverride =
...
export interface BirouterCombo {
  id: string;
  name: string;
  strategy: string;
  active: boolean;
  compressionOverride: BirouterCompressionOverride;
}

/**
 * Fetch the active routing combo list from a running Birouter instance.
 *
...
export async function listCombos(
  ...
  for (const raw of rawList) {
    ...
    const compressionOverride = VALID_COMPRESSION_OVERRIDES.has(rawOverride)
      ? (rawOverride as BirouterCompressionOverride)
      : "";
...
/**
 * Options for `createBirouterComboConfig`.
 * Mirrors the subset of combo fields exposed by the Birouter `/api/combos`
 * PATCH / POST payload that are safe to set programmatically.
 */
export interface BirouterComboConfigOptions {
...
/**
 * Build a typed combo payload suitable for Birouter's management API.
 *
 ...
 * ```
 */
export function createBirouterComboConfig(
  options: BirouterComboConfigOptions
): Record<string, unknown> {
 ...
 */
export interface BirouterRoleOverrides {
...
/** Per-role binding used by `createBirouterAgentBlock`. */
export interface BirouterAgentRole extends BirouterRoleOverrides {
  /** Birouter model id, e.g. `"claude-sonnet-4-5-thinking"`. */
  modelId: string;
...
/** Options for `createBirouterAgentBlock`. */
export interface BirouterAgentBlockOptions {
  /** Per-role bindings. Keys become entries under OpenCode's `agent` block. */
  roles: Record<string, BirouterAgentRole>;
}

/** Single entry inside the emitted OpenCode `agent` block. */
export interface OpenCodeAgentEntry extends BirouterRoleOverrides {
  /** Always emitted as `"birouter/<modelId>"`. */
  model: string;
...
function buildAgentEntry(role: BirouterAgentRole): OpenCodeAgentEntry | undefined {
  if (!role || typeof role.modelId !== "string") return undefined;
  const modelId = role.modelId.trim();
  if (!modelId) return undefined;
  const entry: OpenCodeAgentEntry = { model: `${BIROUTER_PROVIDER_KEY}/${modelId}` };
 ...
 * specific Birouter model. Useful for `.opencode/agent/*.md` defaults and
 ...
 * @example
 * ```ts
 * const agentBlock = createBirouterAgentBlock({
 *   roles: {
 ...
 *   },
 * });
 * // -> { build: { model: "birouter/claude-sonnet-4-5-thinking", temperature: 0.2 }, ... }
 * ```
 */
export function createBirouterAgentBlock(
  options: BirouterAgentBlockOptions
...
/**
 * Per-mode binding used by `createBirouterModesBlock`.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `BirouterAgentRole` + `createBirouterAgentBlock`. This
 * type and the corresponding helper are kept for back-compat with configs
 * still using `mode:`.
 */
export interface BirouterMode extends BirouterAgentRole {}

/**
 * Options for `createBirouterModesBlock`.
 *
 * @deprecated See `BirouterMode`.
 */
export interface BirouterModesBlockOptions {
  /** Per-mode bindings. Keys become entries under OpenCode's deprecated top-level `mode` block. */
  modes: Record<string, BirouterMode>;
}
 ...
 * a specific Birouter model. Emits the same shape as the `agent` block since
 * OpenCode's schema treats them identically (both reference `AgentConfig`).
 *
 * Modes with empty / missing `modelId` are skipped.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `createBirouterAgentBlock`. This helper is kept for
 * back-compat with configs still using `mode:`.
 *
 * @example
 * ```ts
 * const modesBlock = createBirouterModesBlock({
 *   modes: {
 ...
 * ```
 */
export function createBirouterModesBlock(
  options: BirouterModesBlockOptions
...
export default createBirouterProvider;
