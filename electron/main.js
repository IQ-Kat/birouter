/**
 * Birouter Electron Desktop App - Main Process
 *
 ...
 * #14 Removed dead birouter:// protocol (no handler existed)
...
function resolveNodeExecutable(env = process.env) {
  ...
  // (e.g. Birouter.app/Contents/MacOS/Birouter). Spawning it with
    ...
    new Promise((r) =>
      setTimeout(() => {
        try {
          ...
          // bare SIGKILL of the direct child, keeping birouter.exe locked).
...
function installUpdate() {
  if (nextServer) {
    ...
    // grandchildren) keeps birouter.exe locked and the updater fails with "file in use".
...
function setupContentSecurityPolicy() {
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    ...
    const csp = [
      ...
      `connect-src 'self' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* wss://localhost:* wss://127.0.0.1:* https://*.birouter.app https://*.birouter.dev`,
      scriptSrc,
...
function startNextServer() {
  ...
  if (changed) {
    serverEnv.BIROUTER_BOOTSTRAPPED = "true";
    try {
      fs.mkdirSync(dataDir, { recursive: true });
      const lines = [
        "# Auto-generated by Birouter bootstrap",
        "",
  ...
  // BIROUTER_MEMORY_MB or a pre-set --max-old-space-size still wins. Mirrors
  ...
  const serverNodeOptions = (() => {
    ...
    const explicit = parseInt(serverEnv.BIROUTER_MEMORY_MB, 10);
    let heapMb;
  ...
  nextServer.stdout?.on("data", (data) => {
    ...
    if (text.includes("Ready") || text.includes("started") || text.includes("listening")) {
      ...
      const isHeadless =
        ...
        process.env.BIROUTER_HEADLESS === "true";
      if (isHeadless && !global.loggedHeadlessReady) {
        global.loggedHeadlessReady = true;
        console.log("\n\x1b[32m✔ Birouter Headless CLI Server is ready and listening!\x1b[0m");
...
function stopNextServer() {
  if (nextServer) {
    ...
    // (birouter.exe-as-node) spawns grandchildren that a bare SIGTERM leaves alive,
    // holding a lock on birouter.exe and blocking updates.
    killProcessTree(nextServer, { signal: "SIGTERM" });
    nextServer = null;
  }
}
