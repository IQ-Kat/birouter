...
<!-- skill:custom-start -->
<!-- Migrated from skills/birouter-routing/SKILL.md (preserved curated content) -->

# Birouter — Routing & Combos
...
```bash
curl -X POST $BIROUTER_URL/api/combos \
  -H "Authorization: Bearer $BIROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "my-combo",
...
```bash
# Activate
curl -X PUT $BIROUTER_URL/api/combos/{id}/toggle \
  -H "Authorization: Bearer $BIROUTER_KEY" \
  -d '{ "enabled": true }'
```
...
```bash
curl $BIROUTER_URL/api/combos/{id}/metrics \
  -H "Authorization: Bearer $BIROUTER_KEY"
```
...
```bash
curl -X POST $BIROUTER_URL/api/routing/simulate \
  -H "Authorization: Bearer $BIROUTER_KEY" \
  -H "Content-Type: application/json" \
...
```
birouter_list_combos     → list all combos
birouter_switch_combo    → enable/disable a combo
birouter_set_routing_strategy → change strategy at runtime
birouter_simulate_route  → dry-run routing decision
birouter_best_combo_for_task → get recommendation by task type
```
