...
description: Step-by-step agent workflow to configure the OpenAI Codex CLI on any machine (Linux, macOS, Windows) to use Birouter as backend. Detects OS and shell, writes config.toml and 7 named profiles, sets environment variables, and verifies the setup.
---

# /config-codex-cli — Codex CLI Configuration Workflow

Configure the Codex CLI on this machine to use an Birouter instance as backend.

After this skill completes, `codex` will use Birouter by default with `cx/gpt-5.5` (xhigh reasoning), 7 named profiles for quick switching, and proper token limits configured for each model.
...
1. **Birouter host** — the IP or hostname of the Birouter server (e.g. `192.168.0.1`, `100.x.x.x` for Tailscale, or `localhost`)
2. **Birouter API key** — the API key for Birouter (starts with `sk-`)

Store these as local variables for the rest of the skill:

- `BI_HOST` = the host the user provided (no trailing slash, no port — port 20128 is appended by this skill)
- `BI_KEY` = the API key
...
Replace `<BI_HOST>` with the value collected in Step 0.
...
name                 = "Birouter"
base_url             = "http://<BI_HOST>:20128/v1"
...
```bash
grep -l "BIROUTER_API_KEY" ~/.bashrc ~/.zshrc ~/.bash_profile 2>/dev/null
```

If the variable already exists in any profile file, update the value in-place instead of appending a duplicate.

**Block to append (replace `<BI_KEY>` with the key collected in Step 0):**

```bash
# Birouter API key — used by Codex CLI (env_key = "BIROUTER_API_KEY" in config)
export BIROUTER_API_KEY="<BI_KEY>"
...
```fish
set -Ux BIROUTER_API_KEY "<BI_KEY>"
...
```powershell
[System.Environment]::SetEnvironmentVariable("BIROUTER_API_KEY", "<BI_KEY>", "User")
...
echo $BIROUTER_API_KEY              # must print the key
echo $CLAUDE_CODE_MAX_OUTPUT_TOKENS  # must print 65536

# Verify Codex picks up the provider
codex config get model_provider      # must print: birouter
...
- Run `curl http://<BI_HOST>:20128/v1/models` to confirm Birouter is reachable
- Confirm the key is correct with `echo $BIROUTER_API_KEY`
...
Codex CLI deprecated `wire_api = "chat"` in February 2026. Birouter bridges the gap transparently:

```
Codex CLI  →  POST /v1/responses  →  Birouter  →  POST /chat/completions  →  DeepSeek / Mistral / any provider
```

All profiles use the same `wire_api = "responses"`. Birouter handles translation for every upstream provider.
...
_Full reference: `docs/guides/CODEX-CLI-CONFIGURATION.md` in the Birouter repository._
