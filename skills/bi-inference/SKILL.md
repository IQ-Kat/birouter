...
<!-- skill:custom-start -->
<!-- Aggregated from: birouter-chat, birouter-image, birouter-tts, birouter-stt, birouter-embeddings, birouter-web-search, birouter-web-fetch -->
...
- `POST $BIROUTER_URL/v1/chat/completions` — OpenAI format
- `POST $BIROUTER_URL/v1/messages` — Anthropic Messages format
- `POST $BIROUTER_URL/v1/responses` — OpenAI Responses API

### Discover

```bash
curl $BIROUTER_URL/v1/models | jq '.data[].id'
```
...
```bash
curl -X POST $BIROUTER_URL/v1/chat/completions \
...
```bash
curl -X POST $BIROUTER_URL/v1/messages \
  -H "Authorization: Bearer $BIROUTER_KEY" \
...
- `POST $BIROUTER_URL/v1/images/generations` — Text-to-image
- `POST $BIROUTER_URL/v1/images/edits` — Image edit (mask)
- `POST $BIROUTER_URL/v1/images/variations` — Variations

### Discover

```bash
curl $BIROUTER_URL/v1/models/image | jq '.data[]'
```
...
```bash
curl -X POST $BIROUTER_URL/v1/images/generations \
  -H "Authorization: Bearer $BIROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "dall-e-3",
...
- `POST $BIROUTER_URL/v1/audio/speech` — returns binary audio (mp3/opus/wav/flac)

### Discover

```bash
curl $BIROUTER_URL/v1/models/tts | jq '.data[]'
```
...
```bash
curl -X POST $BIROUTER_URL/v1/audio/speech \
  -H "Authorization: Bearer $BIROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "tts-1",
    "input": "Hello from Birouter.",
    "voice": "alloy",
    "response_format": "mp3"
  }' --output speech.mp3
```
...
- `POST $BIROUTER_URL/v1/audio/transcriptions` — multipart upload, returns text
- `POST $BIROUTER_URL/v1/audio/translations` — transcribe + translate to English

### Discover

```bash
curl $BIROUTER_URL/v1/models/stt | jq '.data[]'
```

### Example

```bash
curl -X POST $BIROUTER_URL/v1/audio/transcriptions \
  -H "Authorization: Bearer $BIROUTER_KEY" \
  -F "file=@audio.mp3" \
  -F "model=whisper-1" \
...
- `POST $BIROUTER_URL/v1/embeddings`

### Discover

```bash
curl $BIROUTER_URL/v1/models/embedding | jq '.data[]'
```
...
```bash
curl -X POST $BIROUTER_URL/v1/embeddings \
  -H "Authorization: Bearer $BIROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
- `POST $BIROUTER_URL/v1/web/search` — unified search format

### Discover

```bash
curl $BIROUTER_URL/v1/models/web | jq '.data[] | select(.kind == "webSearch")'
```

### Example

```bash
curl -X POST $BIROUTER_URL/v1/web/search \
  -H "Authorization: Bearer $BIROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "tavily/search",
    "query": "Birouter github latest release",
    "max_results": 5,
    "include_answer": true
  }'
```
...
- `POST $BIROUTER_URL/v1/web/fetch`

### Discover

```bash
curl $BIROUTER_URL/v1/models/web | jq '.data[] | select(.kind == "webFetch")'
```

### Example

```bash
curl -X POST $BIROUTER_URL/v1/web/fetch \
  -H "Authorization: Bearer $BIROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "jina/reader",
