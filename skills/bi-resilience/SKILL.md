...
<!-- skill:custom-start -->
<!-- Migrated from skills/birouter-monitoring/SKILL.md (preserved curated content) -->

# Birouter — Monitoring & Health
...
```bash
curl $BIROUTER_URL/api/monitoring/health \
  -H "Authorization: Bearer $BIROUTER_KEY"
```
...
```bash
curl $BIROUTER_URL/api/providers/metrics \
  -H "Authorization: Bearer $BIROUTER_KEY"
```
...
## Via MCP (if Birouter is your MCP server)

```
birouter_get_health            → full system health snapshot
birouter_get_provider_metrics  → p50/p95/p99 + circuit state per provider
birouter_get_session_snapshot  → cost, tokens, errors for current session
birouter_check_quota           → quota balance + percent remaining + reset time
birouter_db_health_check       → diagnose + auto-repair database drift
```

## Quota check

```bash
curl $BIROUTER_URL/api/quota \
  -H "Authorization: Bearer $BIROUTER_KEY"
```
...
```bash
curl -X POST $BIROUTER_URL/api/budget/guard \
  -H "Authorization: Bearer $BIROUTER_KEY" \
...
Birouter logs every MCP tool call to `mcp_audit` table. Query via API:

```bash
curl "$BIROUTER_URL/api/mcp/status" \
  -H "Authorization: Bearer $BIROUTER_KEY"
```
...
- `503` on health endpoint → Birouter is starting up; retry in 5s
