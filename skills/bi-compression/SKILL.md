...
<!-- skill:custom-start -->
<!-- Migrated from skills/birouter-compression/SKILL.md (preserved curated content) -->
...
Birouter compresses token payloads before forwarding to providers. No code changes required — set it once, it applies to all requests transparently.
...
```bash
curl $BIROUTER_URL/api/settings/compression \
...
```bash
curl -X PUT $BIROUTER_URL/api/settings/compression \
  -H "Authorization: Bearer $BIROUTER_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST $BIROUTER_URL/api/compression/preview \
  -H "Authorization: Bearer $BIROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "mode": "rtk",
...
When Birouter is used with browser/Playwright MCP tools, it automatically compresses verbose accessibility-tree tool results. Enabled by default; configure thresholds:
...
```
birouter_compression_status     → current settings + savings analytics
birouter_compression_configure  → update mode/threshold/language
birouter_set_compression_engine → switch engine at runtime
```
