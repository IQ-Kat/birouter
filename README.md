...
<sub>Enforced in the hot path **before** the request leaves Birouter, with per-(key, model) caps + session stickiness for prompt-cache integrity (now with a per-combo / global disable toggle). 📖 [Quota Sharing Engine](docs/routing/QUOTA_SHARE.md)</sub>
...
- **🌍 Deployment & ops** — reverse-proxy `basePath` deployment (`BIROUTER_BASE_PATH`, e.g. serving Birouter under `/birouter/`), browser-language auto-detect on first visit, per-API-key device/connection tracking (IP+UA fingerprint, masked, in-memory only), root-less MITM cert trust for user-namespaced containers (`BIROUTER_NO_SUDO`), and server-side configured-only / available-only filters on the Free Provider Rankings page. → [Environment](docs/reference/ENVIRONMENT.md)
