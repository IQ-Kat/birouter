...
TPROXY transparent decrypt is Birouter's **5th capture mode** for the
...
configuration change** — but the process must trust the dynamic CA Birouter
...
- Generates a long-lived CA via `generateMitmCa()` (CN `"Birouter MITM CA"`,
...
`birouter-tproxy-ca.crt` (constant `TPROXY_CA_CERT_NAME`) — kept separate from
the static MITM cert's slot (`birouter-mitm.crt`) so the two never clobber each
other.
...
| **Dedicated CA slot**            | The dynamic CA installs to `birouter-tproxy-ca.crt`, never clobbering the static MITM cert.                                                                                                                                                                                                                                                                        |
...
host means anything Birouter intercepts can be decrypted. It is gated behind the
...
- Confirm the process trusts the dynamic CA. The CA is installed under
  `birouter-tproxy-ca.crt`; apps with their own trust store (Firefox/Chrome NSS)
...
mode reverts the rules; if Birouter was killed mid-session, use the AgentBridge
