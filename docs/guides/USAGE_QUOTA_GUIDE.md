...
> **TL;DR**: Birouter tracks every request's token usage, computes cost, enforces per-API-key quota, and surfaces analytics in the dashboard. This guide explains how it all works.
...
Every request that flows through Birouter generates a **usage record** that captures:
...
For providers that don't return usage (some web-cookie providers), Birouter **estimates** tokens using a `~4 chars per token` heuristic (see `open-sse/services/autoCombo/pipelineRouter.ts`).

### Cached Tokens

Birouter tracks `cached_tokens` separately from `prompt_tokens` because:
...
| Tool                   | Description                                        |
| ---------------------- | -------------------------------------------------- |
| `birouter_cost_report` | Generates a per-key cost report for a given period |
| `birouter_check_quota` | Returns current quota status for an API key        |

Example agent invocation:

```json
{
  "tool": "birouter_cost_report",
  "args": { "period": "week" }
}
```
