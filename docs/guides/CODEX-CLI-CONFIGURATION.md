---
title: "Codex CLI — Configuration with Birouter"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Codex CLI — Configuration with Birouter

Complete guide for using the Codex CLI pointed at Birouter as an OpenAI-compatible backend.
...
export BIROUTER_API_KEY="<YOUR_KEY>"
```
...
**Birouter solves this transparently:**

```
Codex CLI
  → wire_api = "responses"
  → POST /v1/responses (Birouter)
    → Birouter Responses ↔ Chat Completions transformer
    → POST /chat/completions (DeepSeek / Mistral / GLM / Kimi / any provider)
```

You never need a separate translation proxy when using Birouter. **All models use `wire_api = "responses"`** — Birouter handles the rest.
...
| Model                                | Birouter ID                          | Context window         | `auto_compact` | `tool_output_limit` |
| ------------------------------------ | ------------------------------------ | ---------------------- | -------------- | ------------------- |
...
All Codex models in Birouter use the `cx/` prefix:

| Codex CLI name          | Birouter model     |
| ----------------------- | ------------------ |
...
Other providers use their own prefix (`kmc/`, `glm/`, `ds/`, `ollamacloud/`, `opencode-go/`, `mistral/`) — the prefix matches the Birouter provider alias.
...
## Generating profiles automatically with `birouter setup-codex`
...
birouter setup-codex --codex-home /path/to/.codex
```
...
Birouter can also **auto-sync** these same profile files after a successful provider model discovery/import changes the live catalog. This is **opt-in and off by default**: toggle it from the **CLI Code dashboard** ("CLI profile auto-sync" → Codex), or set `BIROUTER_AUTO_SYNC_CODEX_PROFILES=true` (it also honors `CLI_ALLOW_CONFIG_WRITES`, on by default). When enabled it only writes separate `~/.codex/*.config.toml` profile files; it never changes the active/default `~/.codex/config.toml`, Codex-lb settings, auth, or provider selection.

---

## Launching Codex with `birouter launch-codex`
...
[model_providers.birouter.http_headers]
"X-Custom-Header" = "value"

# Headers read from env vars
[model_providers.birouter.env_http_headers]
"X-Trace-Id" = "TRACE_ID"

# Extra URL query params (useful for Azure api-version)
[model_providers.birouter.query_params]
...
Verify the model exists in Birouter with the correct prefix. Use `birouter models list` or open `/dashboard/providers/<provider>`.

**`Authentication error`**
Confirm `BIROUTER_API_KEY` is exported: `echo $BIROUTER_API_KEY`.

**`Connection refused`**
Verify Birouter is running and the `base_url` host/port is correct for your network (local vs Tailscale vs VPS).
