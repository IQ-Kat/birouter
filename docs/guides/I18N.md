...
Birouter supports **42 languages** with full dashboard UI translation, translated documentation, and RTL support for Arabic and Hebrew.
...
Birouter uses a hash-based incremental translator for docs, backed by an
OpenAI-compatible LLM endpoint (typically `cx/gpt-5.4-mini` through Birouter
Cloud):
...
| Variable                           | Purpose                                 |
| ---------------------------------- | --------------------------------------- |
| `BIROUTER_TRANSLATION_API_URL`     | OpenAI-compatible base URL, e.g. `…/v1` |
| `BIROUTER_TRANSLATION_API_KEY`     | bearer token (kept out of logs)         |
| `BIROUTER_TRANSLATION_MODEL`       | model id, e.g. `cx/gpt-5.4-mini`        |
...
- **Chunked batching**: Joins multiple strings with `__BIROUTER_I18N_SEPARATOR__` delimiters to minimize API calls (max 1800 chars per request)
...
**Secondary translator** — uses any OpenAI-compatible LLM API (including Birouter itself) to translate existing `docs/i18n/` markdown files. Best for polishing or re-translating docs with better quality than Google Translate.

```bash
python3 scripts/i18n/i18n_autotranslate.py \
  --api-url http://localhost:2004/v1 \
  --api-key sk-your-key \
  --model gpt-4o
```
...
The `birouter` CLI has its own i18n layer separate from the Next.js dashboard.
...
| -------- | ------------------------ | ------------------------------------- |
| 1        | `--lang` flag            | `birouter --lang de status`           |
| 2        | `BIROUTER_LANG` env var  | `BIROUTER_LANG=ja birouter providers` |
...
```bash
# Set language and save to ~/.birouter/.env (persists across sessions)
birouter config lang set pt-BR

# View current language
birouter config lang get

# List all 42 available languages
birouter config lang list

# JSON output
birouter config lang list --output json
```

The saved preference is written atomically to `~/.birouter/.env` and is loaded by the
...
birouter --lang de providers list
```
...
```bash
# Default: es, fr, de, ja, ar on localhost:2004
