...
Birouter ships as a fully installable Progressive Web App. When you access the dashboard from any mobile browser — Android (Chrome) or iOS (Safari) — you can "Add to Home Screen" and get a native app-like experience with no app store required.

## What Is a PWA?

A Progressive Web App turns the Birouter web dashboard into something that looks and feels like a native mobile app. Once installed, it:
...
4. Birouter appears on your home screen as a standalone app
...
5. Birouter appears on your home screen with the app icon

### Desktop (Chrome / Edge)

1. Open the Birouter dashboard
2. Click the **install icon** in the address bar (or ⋮ → "Install Birouter...")
3. Confirm the prompt
4. Birouter opens as a standalone window — no tabs, no address bar
...
Birouter includes a service worker (`sw.js`) that provides intelligent caching:
...
Birouter provides icons optimized for each platform:
...
```json
{
  "name": "Birouter",
  "short_name": "Birouter",
  "description": "Birouter is an AI gateway for multi-provider LLMs. One endpoint for all your AI providers.",
  "start_url": "/",
  "scope": "/",
  "display": "fullscreen",
  "orientation": "any",
...
- **Cache versioning**: `birouter-pwa-v2` — bump this to force a fresh cache on update
