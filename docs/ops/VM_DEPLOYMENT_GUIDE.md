---
title: "Birouter — Deployment Guide on VM with Cloudflare"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Birouter — Deployment Guide on VM with Cloudflare
...
Complete guide to install and configure Birouter on a VM (VPS) with domain managed via Cloudflare.
...
```bash
mkdir -p /opt/birouter
```

### 2.2 Create environment variables file

```bash
cat > /opt/birouter/.env << 'EOF'
# === Security ===
...
# BIROUTER_PUBLIC_BASE_URL=https://llms.seudominio.com

# === Cloud Sync (optional) ===
# CLOUD_URL=https://cloud.Birouter.online
# NEXT_PUBLIC_CLOUD_URL=https://cloud.Birouter.online
EOF
```
...
```bash
cat > /etc/nginx/sites-available/birouter << 'NGINX'
...
Keep reverse-proxy stream timeouts aligned with your Birouter timeout env vars. If you raise
`FETCH_TIMEOUT_MS` / `STREAM_IDLE_TIMEOUT_MS`, raise `proxy_read_timeout` / `proxy_send_timeout`
above the same threshold.

Birouter uses `NEXT_PUBLIC_BASE_URL` as the canonical browser-facing origin for OAuth
...
`BIROUTER_TRUST_PROXY` if Birouter is not directly reachable by clients and your proxy
...
# Enable Birouter
ln -sf /etc/nginx/sites-available/birouter /etc/nginx/sites-enabled/birouter
...
```bash
docker logs -f birouter          # Real-time stream
docker logs birouter --tail 50   # Last 50 lines
```
...
docker cp birouter:/app/data ./backup-$(date +%F)

# Or compress the entire volume
docker run --rm -v birouter-data:/data -v $(pwd):/backup \
  alpine tar czf /backup/birouter-data-$(date +%F).tar.gz /data
```
...
docker run --rm -v birouter-data:/data -v $(pwd):/backup \
  alpine sh -c "rm -rf /data/* && tar xzf /backup/birouter-data-YYYY-MM-DD.tar.gz -C /"
docker start birouter
```
...
See also [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md) for the in-repo Cloudflare Tunnel walkthrough. The standalone `birouterCloud/` worker lives in a separate companion repo.
