...
- **deps: bump the development group with 10 updates** — routine dependabot dev-dependency bumps. ([#4051](https://github.com/IQ-Kat/birouter/pull/4051))
- **deps(electron): bump electron 42.4.0 → 42.4.1** — ([#4049](https://github.com/IQ-Kat/birouter/pull/4049))
- **ci(deps): bump `actions/setup-node` 4 → 6** — ([#4048](https://github.com/IQ-Kat/birouter/pull/4048))
- **ci(deps): bump `actions/cache` 4.3.0 → 5.0.5** — ([#4047](https://github.com/IQ-Kat/birouter/pull/4047))
- **ci(deps): bump `actions/github-script` 7 → 9** — ([#4046](https://github.com/IQ-Kat/birouter/pull/4046))
- **ci(deps): bump `ossf/scorecard-action` 2.4.0 → 2.4.3** — ([#4045](https://github.com/IQ-Kat/birouter/pull/4045))
- **ci(deps): bump `actions/upload-artifact` 4 → 7** — ([#4044](https://github.com/IQ-Kat/birouter/pull/4044))
...
- **chore(deps):** bump `actions/setup-node` from v4 to v6 + `randomBytes` security fix for cloud agent task IDs. ([#2589](https://github.com/IQ-Kat/birouter/pull/2589))
- **chore(deps):** bump `actions/upload-artifact` from v4 to v7. ([#2588](https://github.com/IQ-Kat/birouter/pull/2588))
...
- **feat(skills):** add 5 CLI skill manifests + AgentSkills / BiSkills dashboard pages. ([#2284](https://github.com/IQ-Kat/birouter/pull/2284))
- **fix(translator):** map `developer` → `system` by default for non-OpenAI-family providers. ([#2281](https://github.com/IQ-Kat/birouter/pull/2281))
- **fix(api/combos):** add API-key-safe `GET /v1/combos` endpoint. ([#2300](https://github.com/IQ-Kat/birouter/pull/2300))
- **fix(embeddings/registry):** add DeepInfra to the embedding provider registry. ([#2298](https://github.com/IQ-Kat/birouter/pull/2298))
- **fix(opencode-zen):** flag `qwen3.6-plus` and `qwen3.6-plus-free` with `targetFormat: "claude"`. ([#2292](https://github.com/IQ-Kat/birouter/pull/2292))
...
- `docs/comparison/BIROUTER_VS_ALTERNATIVES.md`
...
> On the first startup after upgrading, Birouter archives legacy request logs from `DATA_DIR/logs/`, legacy `DATA_DIR/call_logs/`, and `DATA_DIR/log.txt` into `DATA_DIR/log_archives/*.zip`, then removes the deprecated layout and switches to the new unified artifact format under `DATA_DIR/call_logs/`.
...
```bash
# npm
npm install -g birouter@3.0.0

# Docker
docker pull IQ-Kat/birouter:3.0.0
...
> Sprint: Fix SSE biModel data loss, merge per-protocol model compatibility.
