...
<img src="../../screenshots/MainBirouter.png" alt="Birouter 仪表板" width="820"/>
...
**~1.6B 有记录的免费 Token/月** — 首月通过注册奖励最高可达 **~2.1B** — 聚合所有免费层的配额，再加上永久免费、无上限的供应商，而上述压缩进一步延长每一分 Token。([统计方法 →](../../reference/FREE_TIERS.md#tldr--how-much-free-inference-does-birouter-actually-aggregate))
...
**问题、供应商技巧、路线图和支持 → [Discord](https://discord.gg/EkzRkpzKYt) · [Telegram](https://t.me/birouterOficial) · WhatsApp [🌍 全球](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t) / [🇧🇷 巴西](https://chat.whatsapp.com/BTGJXIyjeNIIgExvTMGGhI)**
...
[**🚀 快速开始**](#-quick-start) • [**🎯 组合策略**](#-combos--the-flagship) • [**🌐 供应商**](#-231-ai-providers--50-free) • [**🔌 CLI 与 MCP**](#-full-cli--a2a--mcp) • [**🗜️ 压缩**](#%EF%B8%8F-save-1595-tokens--automatically) • [**🌍 网站**](https://Birouter.online)

[💥 承诺](#-the-promise) • [🤔 为什么](#-why-birouter) • [🏆 优势](#-what-sets-birouter-apart) • [🤖 兼容 CLI](#-compatible-clis--coding-agents) • [🖥️ 运行平台](#%EF%B8%8F-where-birouter-runs--anywhere) • [🔒 隐私](#-private--local-first) • [🎬 实际演示](#-birouter-in-action) • [📚 探索更多](#-explore-more) • [📧 支持](#-support--community)
...
> 手动堆叠免费层很痛苦 — 数十个 SDK、数十个速率限制，且不清楚自己到底有多少配额。Birouter 将 **40+ 供应商池 / 500+ 模型**的**有记录**免费层聚合为一个真实数字，并在仪表板上实时展示 (`/dashboard/free-tiers`)。
...
> 一个端点。**231 个供应商。** 永远不要停止构建 — 让 Birouter 选择最便宜且有效的方案。
...
> **Combo** 是 Birouter **自动**路由的模型链。配额用尽、供应商失败或成本飙升 — Combo 自动滑动到下一个模型。**这就是 Birouter 不可中断的原因。** 🛡️

### ⚡ 零配置 — 只需使用 `auto`

无需创建 Combo。将模型设置为 `auto`（或变体），Birouter 会根据您连接的供应商实时评分构建虚拟 Combo：
...
<sub>📊 与 LiteLLM、OpenRouter 和 Portkey 的详细比较 → [`docs/comparison/BIROUTER_VS_ALTERNATIVES.md`](../../comparison/BIROUTER_VS_ALTERNATIVES.md)</sub>
...
- **🛰️ 远程模式** — 通过范围访问令牌从任何机器驱动远程 Birouter（`birouter connect` / `birouter contexts` / `birouter tokens`）。→ [Remote Mode](../../guides/REMOTE-MODE.md)
...
- **⚡ 本地性能与基础设施** — 一键本地 Redis 启动器（`birouter redis up`，加仪表板 Redis 面板）、一键 **Cloudflare Workers** 和 **Deno Deploy** 中继部署器（连接到代理池），以及可选的 Bifrost Go 边车（卸载最热的中继路径，`BIFROST_BASE_URL`，超时时自动回退到 TypeScript 路径）。→ [Environment](../../reference/ENVIRONMENT.md)
...
<sub>📖 所有 16+ 工具的逐个设置 → [`docs/reference/CLI-TOOLS.md`](../../reference/CLI-TOOLS.md) · 🧩 OpenCode 插件 → [`@birouter/opencode-provider`](https://www.npmjs.com/package/@birouter/opencode-provider)</sub>
...
| 平台                     | 安装方式                                    | 亮点                                          |
| ------------------------ | ------------------------------------------- | --------------------------------------------- |
| 📦 **npm（全局）**       | `npm install -g birouter`                   | 一条命令，任何操作系统                        |
| 🐳 **Docker**            | `docker run … IQ-Kat/birouter`              | 多架构 AMD64 + ARM64                          |
| 🖥️ **桌面（Electron）**  | `npm run electron:build`                    | 原生窗口 + 系统托盘 — Windows / macOS / Linux |
| 💪 **ARM**               | 原生 `arm64`                                | Raspberry Pi、ARM 服务器、Apple Silicon       |
| 📱 **Android（Termux）** | `pkg install nodejs-lts && npx -y birouter` | **在手机上**运行，24/7，无需 Root             |
...
> 您的密钥、您的机器、您的数据。Birouter 是**本地代理** — 从不向外部报告。

- 🏠 **100% 在您的硬件上运行** — npm、Docker、桌面或手机。请求路径中不经过任何 Birouter 云。
...
> Birouter 不仅仅是服务器 — 它拥有 **60+ 命令**的**完整命令行控制台**，以及开放的代理协议，AI 代理可以**自行**驱动 Birouter。

### ⌨️ 真正的 CLI（不仅仅是 `start`）

```bash
birouter               # 启动网关 + 仪表板（端口 20128）
birouter chat          # 交互式 TUI 聊天客户端（斜杠命令：/model /combo /skill /memory）
birouter setup         # 引导式首次运行向导
birouter doctor        # 诊断供应商、端口、原生依赖
```

### 🛰️ 远程模式 — 在此处运行 CLI，Birouter 在 VPS 上

Birouter 在服务器上？通过**相同的 CLI** 从笔记本电脑驱动它。登录一次，使用范围访问令牌；每个命令随后都指向远程。

```bash
birouter connect 192.168.0.15            # 密码 → 范围令牌，保存为上下文
birouter models list                     # ← 在远程服务器上运行
birouter configure codex                 # ← 选择远程模型，写入本地 Codex 配置文件
birouter tokens create --name ci --scope read   # 为其他机器铸造更窄范围的令牌
birouter contexts use default            # ← 切换回本地服务器
```
...
### 🤝 连接代理 — 代理自行控制 Birouter

通过 **MCP** 或 **A2A** 暴露 Birouter，任何有能力的代理都能获得整个网关的密钥 — 路由、供应商、Combo、缓存、压缩、记忆 — 自主运作。

| 协议                | 端点                                            | 用途                                              |
| ------------------- | ----------------------------------------------- | ------------------------------------------------- |
| 🧰 **MCP（stdio）** | `birouter --mcp`                                | 接入 Claude Desktop、Cursor 等 MCP 客户端         |
...
```bash
# 给 Claude Code 完整的 Birouter 工具集，通过 MCP：
...
> **为什么用很多 Token 而不用少量 Token？** 每个请求**透明地**通过 Birouter 的压缩流水线 — 无需更改客户端。它现在是**9 个可组合引擎的堆栈**，按顺序运行并按路由 Combo 混合搭配 — 基于 [RTK](https://github.com/rtk-ai/rtk)、[Caveman](https://github.com/JuliusBrussee/caveman)（⭐ 51K+）、[LLMLingua-2](https://github.com/microsoft/LLMLingua) 和 [Troglodita](https://github.com/leninejunior/troglodita)（PT-BR）的理念构建。
...
- **🎛️ 压缩决策位置** _（优先级，高 → 低）_ — 每请求 `x-birouter-compression` 标头 › 路由 Combo 覆盖 › 活动命名配置文件 › 自适应/自动触发 › 面板默认 › 关闭。应用的方案在 `X-Birouter-Compression: <mode>; source=<source>` 响应标头中回显。
...
您应该能看到已连接的模型列表。🎉 搞定了 — 开始编码，Birouter 会自动路由和故障转移。

如果您的客户端无法发送自定义标头，Birouter 还提供令牌化兼容别名：
...
> 🎬 **制作了关于 Birouter 的视频？** 通过链接打开 [issue](https://github.com/IQ-Kat/birouter/issues/new) 或 [discussion](https://github.com/IQ-Kat/birouter/discussions) — 我们会在这里展示它。
...
> 💡 仪表板上的"成本"是**节省追踪器**，不是账单 — Birouter 从不向您收费。使用免费模型显示的"$290 总成本"意味着**节省了 $290**。
...
🇷🇺 🇨🇳 🇮🇷 🇨🇺 🇹🇷 在被封锁的地区？Birouter 的 **3 层代理**（全局/每供应商/每连接）代理 API 请求、OAuth 流程、连接测试、Token 刷新和模型同步。
...
| `DATA_DIR`        | `~/.birouter` | 数据库和配置存储  |

**Birouter 会向我收费吗？** 不会 — 它是免费的开源软件，运行在您的机器上。您只直接向付费供应商付款。Birouter 没有计费系统。
...
- 🌍 **网站**：[Birouter.online](https://Birouter.online)
- 🐙 **GitHub**：[github.com/IQ-Kat/birouter](https://github.com/IQ-Kat/birouter)
- 🐛 **Issues**：[报告 Bug](https://github.com/IQ-Kat/birouter/issues)（请附上 `npm run system-info` 输出）
...
- **网站**：[Birouter.online](https://Birouter.online)
- **包**：[npmjs.com/package/Birouter](https://www.npmjs.com/package/Birouter)
...
| [Remote Mode](../../guides/REMOTE-MODE.md)                      | 通过范围访问令牌从笔记本电脑 CLI 驱动远程 Birouter（VPS）               |
| [Claude Code Config](../../guides/CLAUDE-CODE-CONFIGURATION.md) | 使用 `launch` + 每模型配置文件将 Claude Code 指向 Birouter              |
...
| [Termux Guide](../../guides/TERMUX_GUIDE.md)              | 在 Android 上通过 Termux 运行 Birouter                     |
...
> Birouter 由充满激情的开源社区塑造。以下个人做出了卓越贡献，直接影响了项目的质量、稳定性和影响力。**感谢你们。**
...
> 🙏 这些贡献者的功能、Bug 修复和基础设施改进是 Birouter 可靠且功能丰富的**核心部分**。每个 Pull Request、每个测试用例、每个 i18n 翻译文件都很重要。开源由像他们这样的人构建。
...
Birouter 站在巨人的肩膀上。它始于 **[9router](https://github.com/decolua/9router)** 的一个 Fork 和 Go 项目 **[CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)** 的 TypeScript 移植 — 从此，下面的每个子系统都受到了先行开源项目的启发。每个项目都塑造了 Birouter 的具体部分。这是我们对所有项目的感谢。🙏
...
Birouter 是免费且开源的，在公开环境中构建和维护。如果它为您节省了时间或金钱，请考虑支持其开发：

- ⭐ **为仓库加星** — 这确实有助于提高能见度
- 💖 **[GitHub Sponsors](https://github.com/sponsors/diegosouzapw)** — 资助持续维护和新供应商
- 🐛 **在 [Discussions](https://github.com/IQ-Kat/birouter/discussions) 中报告 Bug 和分享反馈**
