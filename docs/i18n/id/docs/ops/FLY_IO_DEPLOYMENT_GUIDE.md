# Panduan Deployment Birouter di Fly.io
...
Dokumen ini menjelaskan metode deployment Birouter di Fly.io yang telah terbukti berhasil, mencakup dua skenario utama:
...
Dokumen ini disusun berdasarkan konfigurasi yang telah diverifikasi pada proyek saat ini, dengan nama aplikasi `birouter`.
...
- Alamat akses: `https://birouter.fly.dev/`
...
- `app = 'birouter'` menentukan aplikasi Fly mana yang menjadi target deployment
...
- Jika sebelumnya Anda menggunakan nama lain seperti `oroute`, jangan sampai tertukar dengan `birouter`
...
Jika Anda sudah mengganti nama aplikasi, ganti `birouter` dengan nama Anda.
...
Parameter-parameter berikut telah digunakan secara nyata pada aplikasi `birouter` saat ini:
...
- Berlaku untuk proyek saat ini yaitu `birouter`
...
```powershell
flyctl secrets set INITIAL_PASSWORD=kata-sandi-kuat-anda -a birouter
```
...
- Apakah aplikasi yang sedang dilihat adalah `birouter`
...
Jika repositori saat ini adalah fork dan Anda ingin menyinkronkan pembaruan dari upstream `https://github.com/IQ-Kat/birouter`, ikuti alur berikut.
...
- Anda membuka aplikasi yang salah, misalnya `oroute`, bukan `birouter`
