...
Referensi lengkap untuk semua titik akhir API Birouter.
...
| ----------------------- | ---------- | ------------------------------------------------ |
| `X-Birouter-No-Cache`   | Permintaan | Atur ke `true` untuk melewati cache              |
| `X-Birouter-Progress`   | Permintaan | Atur ke `true` untuk event progres               |
...
| `X-Birouter-Cache`      | Respons    | `HIT` atau `MISS` (non-streaming)                |
| `X-Birouter-Idempotent` | Respons    | `true` jika dideduplikasi                        |
| `X-Birouter-Progress`   | Respons    | `enabled` jika pelacakan progres aktif           |
| `X-Birouter-Session-Id` | Respons    | ID sesi efektif yang digunakan Birouter          |
