# Dokumentasi Server MCP Birouter (Bahasa Indonesia)
...
Birouter MCP sudah tersedia secara bawaan. Jalankan dengan:
...
| Alat                           | Deskripsi                                       |
| :----------------------------- | :---------------------------------------------- |
| `birouter_get_health`          | Kesehatan gateway, pemutus sirkuit, uptime      |
| `birouter_list_combos`         | Semua combo yang dikonfigurasi beserta modelnya |
| `birouter_get_combo_metrics`   | Metrik performa untuk combo tertentu            |
| `birouter_switch_combo`        | Ganti combo aktif berdasarkan ID/nama           |
| `birouter_check_quota`         | Status kuota per penyedia atau semua penyedia   |
| `birouter_route_request`       | Kirim penyelesaian chat melalui Birouter        |
| `birouter_cost_report`         | Analitik biaya untuk periode waktu tertentu     |
| `birouter_list_models_catalog` | Katalog model lengkap beserta kemampuannya      |

## Alat Lanjutan (8)

| Alat                              | Deskripsi                                                              |
| :-------------------------------- | :--------------------------------------------------------------------- |
| `birouter_simulate_route`         | Simulasi routing percobaan dengan pohon fallback                       |
| `birouter_set_budget_guard`       | Anggaran sesi dengan tindakan degrade/block/alert                      |
| `birouter_set_resilience_profile` | Terapkan preset conservative/balanced/aggressive                       |
| `birouter_test_combo`             | Uji langsung semua model dalam combo melalui permintaan upstream nyata |
| `birouter_get_provider_metrics`   | Metrik terperinci untuk satu penyedia                                  |
| `birouter_best_combo_for_task`    | Rekomendasi kesesuaian tugas beserta alternatifnya                     |
| `birouter_explain_route`          | Jelaskan keputusan routing yang lalu                                   |
| `birouter_get_session_snapshot`   | Status sesi lengkap: biaya, token, kesalahan                           |
