# 🚀 Birouter — The Free AI Gateway (Azərbaycan dili)
...
[🌐 Website](https://Birouter.online) • [🚀 Quick Start](#-quick-start) • [💡 Features](#-key-features) • [📖 Docs](#-documentation) • [💰 Pricing](#-pricing-at-a-glance) • [💬 WhatsApp](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t)
...
_Connect any AI-powered IDE or CLI tool through Birouter — free API gateway for unlimited coding._
...
This generates a `system-info.txt` with your Node.js version, Birouter version, OS details, installed CLI tools (qoder, gemini, claude, codex, antigravity, droid, etc.), Docker/PM2 status, and system packages — everything we need to reproduce your issue quickly. Attach the file directly to your GitHub issue.
...
## 🎯 What Birouter Solves — 30 Real Pain Points & Use Cases

> **Every developer using AI tools faces these problems daily.** Birouter was built to solve them all — from cost overruns to regional blocks, from broken OAuth flows to protocol operations and enterprise observability.
...
- **System Info Report** — `npm run system-info` generates `system-info.txt` with your full environment (Node version, Birouter version, OS, CLI tools, Docker/PM2 status). Attach it when reporting issues for instant triage.
...
- **npm global install** — `npm install -g birouter && birouter` — done
...
- Built-in quick-start for `birouter --mcp` and client onboarding
...
Arch Linux users can install the [AUR package](https://aur.archlinux.org/packages/birouter-bin), which installs Birouter and provides a systemd user service:
...
When you no longer need Birouter, we provide two quick scripts for a clean removal:

| Command                  | Action                                                                              |
| ------------------------ | ----------------------------------------------------------------------------------- |
| `npm run uninstall`      | Removes the system app but **keeps your DB and configurations** in `~/.birouter`.   |
| `npm run uninstall:full` | Removes the app AND permanently **erases all configurations, keys, and databases**. |

> Note: To run these commands, navigate to the Birouter project folder (if you cloned it) and run them. Alternatively, if globally installed, you can simply run `npm uninstall -g birouter`.
...
| `STREAM_IDLE_TIMEOUT_MS` | inherits `REQUEST_TIMEOUT_MS` | Maximum gap between streaming chunks before Birouter aborts the SSE stream                                                                   |

Backward compatibility is preserved: existing `FETCH_TIMEOUT_MS`, `API_BRIDGE_PROXY_TIMEOUT_MS`, and other per-layer timeout vars still work and override the shared baseline.

For Claude Code-compatible upstreams (`anthropic-compatible-cc-*`), Birouter also derives the outbound `X-Stainless-Timeout` header from the resolved fetch timeout so provider-side read timeouts stay aligned with your env configuration.

For third-party Claude Code-compatible reverse proxies, Birouter keeps the default
...
`cache_control`, Birouter does not inject bridge-owned markers.
...
For streaming requests, `FETCH_TIMEOUT_MS` only covers connection setup / waiting for the first upstream response. Once the stream is active, Birouter will only abort on an actual stall (`STREAM_IDLE_TIMEOUT_MS`) or Undici body inactivity (`FETCH_BODY_TIMEOUT_MS`).

If you run Birouter behind Nginx, Caddy, Cloudflare, or another reverse proxy, make sure the proxy
timeouts are also higher than your Birouter stream/fetch timeouts.
...
For Void Linux users, you can build a native package using `xbps-src`. Save this block as `srcpkgs/birouter/template`:
...
Birouter is available as a public Docker image on [Docker Hub](https://hub.docker.com/r/IQ-Kat/birouter).
...
- Quick Tunnels are not auto-restored after an Birouter or container restart. Re-enable them from the dashboard when needed.
- Managed install currently supports Linux, macOS, and Windows on `x64` / `arm64`.
- Managed Quick Tunnels default to HTTP/2 transport to avoid noisy QUIC UDP buffer warnings in constrained container environments. Set `CLOUDFLARED_PROTOCOL=quic` or `auto` if you want a different transport.
- Docker images bundle system CA roots and pass them to managed `cloudflared`, which avoids TLS trust failures when the tunnel bootstraps inside the container.
- SQLite runs in WAL mode. `docker stop` should be allowed to finish so Birouter can checkpoint the latest changes back into `storage.sqlite`.
...
- Set `CLOUDFLARED_BIN=/absolute/path/to/cloudflared` if you want Birouter to use an existing binary instead of downloading one.

**Using Docker Compose with Caddy (HTTPS Auto-TLS):**

Birouter can be securely exposed using Caddy's automatic SSL provisioning. Ensure your domain's DNS A record points to your server's IP.
...
> 🆕 **NEW!** Birouter is now available as a **native desktop application** for Windows, macOS, and Linux.

Run Birouter as a standalone desktop app — no terminal, no browser, no internet required for local models. The Electron-based app includes:

- 🖥️ **Native Window** — Dedicated app window with system tray integration
- 🔄 **Auto-Start** — Launch Birouter on system login
...
When minimized, Birouter lives in your system tray with quick actions:
...
**Zero cost. Never stops coding.** Configure this as one Birouter combo and all fallbacks happen automatically — no manual switching ever.
...
> All models below are **100% free with zero credit card required**. Birouter auto-routes between them when one quota runs out — combine them all for an unbreakable $0 combo.
...
Birouter v3.6 is built as an operational platform, not just a relay proxy.
...
When quota, rate, or health fails, Birouter automatically moves to the next candidate without manual switching.
...
Birouter includes a built-in evaluation framework to test LLM response quality against a golden set. Access it via **Analytics → Evals** in the dashboard.

### Built-in Golden Set

The pre-loaded "Birouter Golden Set" contains test cases for:
...
2. Run `birouter_get_health`.
3. Run `birouter_list_combos`.
...
**Pro Tip:** Use Opus for complex tasks, Sonnet for speed. Birouter tracks quota per model!
...
- Rotation behavior: Birouter routes to the next eligible Codex account automatically.
...
**Pro Tip:** OpenAI-compatible API — works seamlessly with Birouter's format translation!
...
> **Note:** OpenClaw only works with local Birouter. Use `127.0.0.1` instead of `localhost` to avoid IPv6 resolution issues.
...
**Step 1:** Add Birouter as a custom provider:
...
> **Tip:** Add any model available in your Birouter `/v1/models` endpoint to the `models` section. Use the format `provider/model-id` from your Birouter dashboard.
...
- Birouter v1.0.6+ includes fallback validation via chat completions
...
- Name: anything you like (e.g. `Birouter Remote`)
...
1. Birouter opens the Google authorization URL
...
4. Paste that URL into the field shown in the Birouter connection modal
5. Click **"Connect"**
...
Birouter has **218+ features planned** across multiple development phases. Here are the key areas:
...
Special thanks to **[9router](https://github.com/decolua/9router)** by **[decolua](https://github.com/decolua)** — the original project that inspired this fork. Birouter builds upon that incredible foundation with additional features, multi-modal APIs, and a full TypeScript rewrite.
