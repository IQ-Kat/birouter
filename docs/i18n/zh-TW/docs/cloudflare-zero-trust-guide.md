# Guia Completo: Cloudflare Tunnel & Zero Trust (Split-Port) (中文（簡體）)
...
Este guia documenta o padrão ouro de infraestrutura de rede para proteger o **Birouter** e expor sua aplicação de forma segura para a internet, **sem abrir nenhuma porta (Zero Inbound)**.
...
3. Clique em **Add a Tunnel**, escolha **Cloudflared** e dê o nome \`Birouter-VM\`.
...
3. Em **Application name**, coloque \`Painel Birouter\`.
4. Em **Application domain**, coloque \`birouter.seuglobal.com.br\` (O mesmo que você fez na "Rota 2").
5. Clique em **Next**.
...
4. **Name:** \`Anti-Abuso Birouter API\`
...
2. O Birouter só conversa HTTPS saindo (\`cloudflared\`) e não recebendo TCP direto do mundo.
