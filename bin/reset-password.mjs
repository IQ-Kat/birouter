 ...
 *   birouter reset-password
 *
 * Non-interactive / scripted usage (piped stdin, e.g. CI or Docker):
 *   printf 'NewPass123\nNewPass123\n' | birouter reset-password
 *   printf 'NewPass123' | birouter reset-password --password-stdin
 *
 * Resets the admin password for Birouter.
...
async function main() {
  ...
  if (!passwordState.exists) {
    ...
    console.error(`   Make sure Birouter has been started at least once.`);
  ...
  console.log("   Restart Birouter for changes to take effect.\n");
}

main()
  .then(() => {
    // Explicit exit(0) so a caller that imports this module (bin/birouter.mjs
    // routes `birouter reset-password` here) terminates cleanly instead of
