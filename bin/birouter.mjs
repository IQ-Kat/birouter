...
function loadEnvFile() {
  ...
  const loadFile = (envPath) => {
    if (seenEnvPaths.has(envPath)) return false;
  ...
  if (process.env.DATA_DIR) {
    loadFile(join(process.env.DATA_DIR, ".env"));
  }

  loadFile(join(getDefaultDataDir(), ".env"));
  ...
  if (process.env.BIROUTER_CLI_SKIP_REPO_ENV !== "1") {
    loadFile(join(ROOT, ".env"));
  }

  // Chicken-and-egg fix: If DATA_DIR was just loaded from cwd/.env or ROOT/.env,
  // load the .env in DATA_DIR now so that path-specific variables (like STORAGE_ENCRYPTION_KEY) are loaded.
  if (process.env.DATA_DIR) {
    loadFile(join(process.env.DATA_DIR, ".env"));
  }
...
// Generate STORAGE_ENCRYPTION_KEY if not set (persisted to ~/.birouter/.env)
// This ensures the key survives across upgrades and is not regenerated on each install.
// See: https://github.com/IQ-Kat/birouter/issues/1622
//
...
// key or write ~/.birouter/.env — running a read-only command should never
// mutate the data dir.
if (shouldProvisionStorageKey(process.argv)) {
  ...
  if (!process.env.STORAGE_ENCRYPTION_KEY) {
    ...
    // Writing only to ~/.birouter (the container home, not a volume) silently lost the key on
    // container recreation, leaving the persisted encrypted DB undecryptable (regression of #1622).
    const dataDir = process.env.DATA_DIR || join(homedir(), ".birouter");
...
// Apply --lang before Commander parses (program descriptions call t() during setup)
{
  ...
  const langEnv = process.env.BIROUTER_LANG;
...
process.on("exit", () => {
  if (process.env.BIROUTER_NO_UPDATE_NOTIFIER) return;
  ...
  if (_notifier.update) {
    _notifier.notify({
      ...
      message:
        ...
        "Run `npm install -g birouter` or `birouter update --apply`",
    });
  }
});
...
const HAS_SERVE_FLAG = process.argv.some((a) => a.startsWith("--"));

if (NO_ARGS && !HAS_SERVE_FLAG) {
  const { showLaunchMenu } = await import(
    pathToFileURL(join(ROOT, "bin", "cli", "commands", "launch-menu.mjs")).href
  );
  const action = await showLaunchMenu(_pkg.version);

  if (action === "quit") {
    process.exit(0);
  }

  // Rewrite process.argv to simulate the chosen command
  if (action === "serve") {
    // Default: serve + open browser
    process.argv.push("serve");
  } else if (action === "serve-tray") {
    const { spawn } = await import("node:child_process");
    const child = spawn(process.execPath, [__filename, "serve", "--no-open", "--tray"], {
      detached: true,
      stdio: "ignore",
      windowsHide: true,
    });
    child.unref();
    console.log("  \x1b[32m✔ Birouter is running in background (system tray).\x1b[0m");
    console.log("  \x1b[2mClosing terminal...\x1b[0m");
    process.exit(0);
  } else if (action === "help") {
