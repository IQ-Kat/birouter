 ...
 *
 * Purely informational invocations must NOT create `~/.birouter/.env` or write
 * a key — they never touch encrypted storage. Generating a 32-byte key and a
 * `.env` file just to print `birouter --version` (or `--help`) is a surprising
 ...
 *
 * Returns TRUE for everything else, INCLUDING a bare `birouter` (no args) — the
...
export function shouldProvisionStorageKey(argv) {
  ...
  // Bare `birouter` runs the default `serve` command → must provision.
