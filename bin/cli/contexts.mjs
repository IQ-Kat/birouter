...
function defaultConfig() {
  return {
    ...
    contexts: {
      default: { baseUrl: `http://localhost:${process.env.PORT || "2004"}`, apiKey: null },
    },
  };
}
 ...
 * `birouter contexts ...`). For backward compatibility we also read the legacy
 ...
 * store. `overrideName` (from `--context`/`BIROUTER_CONTEXT`) wins when set.
 *
...
export function resolveActiveContext(overrideName) {
  ...
  return { baseUrl: `http://localhost:${process.env.PORT || "2004"}` };
}
