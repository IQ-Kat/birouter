...
const APP_LABEL = "com.birouter.autostart";
const WIN_REG_VALUE = "Birouter";
const LINUX_SERVICE_NAME = "birouter.service";
const LINUX_DESKTOP_NAME = "birouter.desktop";

function resolveCliPath() {
  const candidates = [];
  if (process.argv[1]) candidates.push(process.argv[1]);
  try {
    const which = execSync("command -v birouter 2>/dev/null", { encoding: "utf8" }).trim();
    if (which) candidates.push(which);
  } catch {
    // command -v unavailable
  }
  candidates.push(join(dirname(fileURLToPath(import.meta.url)), "..", "..", "birouter.mjs"));

  for (const candidate of candidates) {
    if (!candidate) continue;
    try {
      const resolved = realpathSync(candidate);
      if (resolved.endsWith("birouter.mjs") && existsSync(resolved)) return resolved;
    } catch {
      // try next candidate
    }
  }
...
function writeLinuxSystemdUnit(cliPath) {
  ...
  const envFile = join(userHomeDir(), ".birouter", ".env");
  const lines = [
    "[Unit]",
    "Description=Birouter AI proxy router",
 ...
 * running process. When the running Birouter cli was itself spawned by the
