...
function ensureBackup(configPath) {
  ...
  const backupDir = path.join(path.dirname(configPath), ".birouter.bak");
...
async function runConfigGetCommand(toolId, opts = {}) {
  if (!toolId) {
    printError("Tool ID required. Usage: birouter config get <tool>");
    return 1;
  }
...
async function runConfigSetCommand(toolId, opts = {}) {
  if (!toolId) {
    printError("Tool ID required. Usage: birouter config set <tool> [options]");
    return 1;
  }
  ...
  if (!apiKey) {
    printError("API key required. Use --api-key or set BIROUTER_API_KEY.");
    return 1;
  }
...
async function runConfigValidateCommand(toolId, opts = {}) {
  if (!toolId) {
    printError("Tool ID required. Usage: birouter config validate <tool>");
    return 1;
  }
...
export async function runConfigLangSetCommand(code, opts = {}) {
  ...
  upsertEnvLine(envPath, "BIROUTER_LANG", code);
  setLocale(code);
  ...
  config
    ...
    .action(async (tool, opts, cmd) => {
      ...
      const exitCode = await runConfigSetCommand(tool, {
        ...opts,
        apiKey: opts.apiKey || globalOpts.apiKey || process.env.BIROUTER_API_KEY,
        baseUrl: opts.baseUrl || globalOpts.baseUrl || process.env.BIROUTER_BASE_URL,
