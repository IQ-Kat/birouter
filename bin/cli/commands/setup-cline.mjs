/**
 * birouter setup-cline — configure the Cline AI coding agent to use Birouter.
 *
 ...
 * writes the CLI-mode files (matching the Birouter dashboard) AND prints the
...
export function resolveClineTarget(opts = {}) {
  ...
  if (opts.remote) baseUrl = stripToRoot(opts.remote);
    ...
    try {
      baseUrl = stripToRoot(
        resolveActiveContext(opts.context ?? process.env.BIROUTER_CONTEXT)?.baseUrl
      );
    } catch {
      /* none */
    }
...
/** Merge Birouter openai-compatible settings into Cline's globalState (Plan + Act). */
...
export function buildClineSecrets(existing, { apiKey }) {
  return { ...(existing || {}), openAiApiKey: apiKey || "sk_birouter" };
}
...
export async function runSetupClineCommand(opts = {}) {
  ...
  printHeading("Birouter → Cline (OpenAI-compatible)");
  ...
  if (dryRun) {
    ...
    console.log(`\n── [dry-run] ${secPath} ── (openAiApiKey: ${apiKey ? "set" : "sk_birouter"})`);
  } else {
...
export function registerSetupCline(program) {
  program
    .command("setup-cline")
    .description(
      "Configure Cline for Birouter: write ~/.cline/data (CLI mode) + print VS Code extension settings"
    )
