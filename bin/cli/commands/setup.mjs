...
async function resolveProviderInput(opts, prompt, nonInteractive) {
  ...
  if (!apiKey) {
    throw new Error("Provider API key is required. Pass --api-key or BIROUTER_API_KEY.");
  }
...
export function registerSetup(program) {
  ...
  // Wire up `birouter setup opencode` subcommand. Kept inside registerSetup
...
export async function runSetupCommand(opts = {}) {
  ...
  try {
    printHeading("Birouter Setup");
    const { db, dbPath } = await openBirouterDb();
