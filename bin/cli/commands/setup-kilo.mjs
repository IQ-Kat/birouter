/**
 * birouter setup-kilo — configure Kilo Code to use Birouter.
 *
 ...
 * This writes BOTH (matching the Birouter dashboard) and prints the UI settings.
 *
...
/** Merge the Birouter openai-compatible provider into Kilo's CLI auth.json. */
...
export async function runSetupKiloCommand(opts = {}) {
  ...
  printHeading("Birouter → Kilo Code (OpenAI-compatible)");
...
export function registerSetupKilo(program) {
  program
    .command("setup-kilo")
    .description(
      "Configure Kilo Code for Birouter: write ~/.local/share/kilo/auth.json (CLI) + VS Code kilocode.* settings"
    )
