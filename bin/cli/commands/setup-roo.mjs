/**
 * birouter setup-roo — configure Roo Code (RooVeterinaryInc.roo-cline) for Birouter.
 *
 ...
 * must support native OpenAI tool-calling (Birouter does).
 */
...
export function buildRooImport({ baseUrl, apiKey, model }) {
  return {
    providerProfiles: {
      currentApiConfigName: "Birouter",
      apiConfigs: {
        Birouter: {
          apiProvider: "openai",
          openAiBaseUrl: baseUrl,
          openAiApiKey: apiKey || "sk_birouter",
          openAiModelId: model,
...
export async function runSetupRooCommand(opts = {}) {
  ...
  const importPath =
    opts.importPath ?? opts["import-path"] ?? join(os.homedir(), ".birouter", "roo-settings.json");
  const vscodePath =
    opts.vscodeSettings ??
    opts["vscode-settings"] ??
    join(os.homedir(), ".config", "Code", "User", "settings.json");

  printHeading("Birouter → Roo Code (OpenAI-compatible)");
      ...
      JSON.stringify(
        {
          ...
          providerProfiles: {
            ...
            apiConfigs: {
              Birouter: {
                ...importDoc.providerProfiles.apiConfigs.Birouter,
                openAiApiKey: apiKey ? "set" : "sk_birouter",
              },
            },
          },
        },
  ...
  printInfo(`  API Key:   <your BIROUTER_API_KEY>`);
...
export function registerSetupRoo(program) {
  program
    .command("setup-roo")
    .description(
      "Configure Roo Code for Birouter: write a Roo import JSON + autoImport pointer + print UI steps"
    )
    ...
    .option("--import-path <path>", "Roo import JSON path (default: ~/.birouter/roo-settings.json)")
    .option(
