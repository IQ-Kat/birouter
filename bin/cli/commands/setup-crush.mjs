/**
 * birouter setup-crush — configure Crush (charmbracelet/crush) for Birouter.
 *
 ...
 * include /v1; the api_key may reference an env var (`$BIROUTER_API_KEY`) so the
...
const API_KEY_REF = "$BIROUTER_API_KEY";
...
export function buildCrushProvider(modelIds, baseUrl) {
  ...
  for (const id of modelIds) {
    ...
    if (!cfg) continue;
    models.push({ id, name: `Birouter: ${id}`, context_window: cfg.ctx });
  }
  return {
    type: "openai-compat",
    base_url: baseUrl,
...
/** Merge the Birouter provider into an existing crush.json (preserve the rest). */
export function mergeCrushConfig(existing, provider) {
  const cfg = existing && typeof existing === "object" ? { ...existing } : {};
  cfg.providers = { ...(cfg.providers || {}), birouter: provider };
  return cfg;
}
...
export async function runSetupCrushCommand(opts = {}) {
  ...
  printHeading("Birouter → Crush (openai-compat)");
  ...
  if (dryRun) {
    ...
    printInfo(
      `[dry-run] ${provider.models.length} model(s) under providers.birouter → ${configPath}`
    );
    return 0;
  }
  mkdirSync(join(configPath, ".."), { recursive: true });
  writeFileSync(configPath, out, "utf8");
  printSuccess(`Wrote ${configPath} (${provider.models.length} models under providers.birouter)`);
  printInfo("Provide the key (config references $BIROUTER_API_KEY):  export BIROUTER_API_KEY=...");
