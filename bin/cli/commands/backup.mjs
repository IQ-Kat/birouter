...
export function registerBackup(program) {
  ...
  // Legacy: `birouter backup` without subcommand still creates a backup
...
async function pruneBackups(backupDir, retention) {
  ...
  try {
    const dirs = readdirSync(backupDir)
      .filter((f) => f.startsWith("birouter-backup-"))
      .sort()
      .reverse();
...
export async function runBackupCommand(opts = {}) {
  ...
  const backupName = safeName ? `birouter-backup-${safeName}` : `birouter-backup-${timestamp}`;
...
async function _uploadBackupToCloud(backupPath, info) {
  ...
  try {
    const boundary = `birouter-backup-${Date.now().toString(36)}-${randomBytes(8).toString("hex")}`;
    const headers = new Headers({
      accept: "application/json",
      "content-type": `multipart/form-data; boundary=${boundary}`,
    });
    const apiKey = process.env.BIROUTER_API_KEY;
    if (apiKey) headers.set("authorization", `Bearer ${apiKey}`);
    const cliToken = process.env.BIROUTER_CLI_TOKEN ?? (await getCliToken());
...
export async function runRestoreCommand(backupId, opts = {}) {
  ...
  if (opts.list || !backupId) {
    ...
    if (!backupId) console.log("\nUsage: birouter restore <backup-id>");
    return 0;
  }

  const safeBackupId = String(backupId).replace(/[/\\]/g, "_");
  const backupPath = join(backupDir, `birouter-backup-${safeBackupId}`);
