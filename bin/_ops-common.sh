# bin/_ops-common.sh — shared helpers for the Birouter ops runbook scripts.
#
...
ops_set_data_dir() {
  BIROUTER_DATA_DIR="$1"
  BIROUTER_SQLITE="${BIROUTER_DATA_DIR}/storage.sqlite"
  BIROUTER_BACKUPS_DIR="${DB_BACKUPS_DIR:-${BIROUTER_DATA_DIR}/db_backups}"
}
ops_set_data_dir "${DATA_DIR:-$HOME/.birouter}"
...
ops_find_snapshot() {
  ...
  for cand in \
    "$id" \
    "$id/" \
    "$BIROUTER_BACKUPS_DIR/$id" \
    "$BIROUTER_BACKUPS_DIR/snapshot_$id"; do
  ...
  if [ -d "$BIROUTER_BACKUPS_DIR" ]; then
    for cand in "$BIROUTER_BACKUPS_DIR"/snapshot_*"$id"*; do
      [ -f "$cand/storage.sqlite" ] && { printf '%s\n' "$cand"; return 0; }
    done
  fi
  ops_die "no snapshot matching '$id' under $BIROUTER_BACKUPS_DIR (run bin/snapshot-data.sh first)"
}
