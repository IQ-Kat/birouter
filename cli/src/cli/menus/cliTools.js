...
async function buildCodexHeader() {
  ...
  const { installed, hasBirouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasBirouter, settings } = result.data;
  ...
  // Extract Birouter custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:Birouter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract Birouter provider config
  const provider = settings?.models?.providers?.["birouter"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("birouter/") ? primary.replace("birouter/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasBirouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use Birouter\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
