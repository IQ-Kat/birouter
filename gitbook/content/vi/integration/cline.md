...
Tích hợp Birouter với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của Birouter.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- Birouter đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint Birouter:
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng Birouter để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua Birouter
...
3. Nhận giải thích AI qua Birouter

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng Birouter để hiểu context và thực hiện thay đổi
...
1. Xác minh Birouter đang chạy: `curl http://localhost:2004/health`
2. Kiểm tra base URL đúng và bao gồm `/v1`
3. Đảm bảo không firewall nào chặn port 2004
...
2. Xác minh Birouter instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs Birouter để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng Birouter cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://birouter.com`
2. Đảm bảo bạn đã cấu hình API key trong Birouter cloud dashboard
...
## Tích hợp với Tính năng Birouter

### Định tuyến Model

Birouter tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, Birouter tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua Birouter dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
