...
**Birouterは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、Birouter自体は100%無料でオープンソースです。**
...
**Birouterはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## Birouterをセルフホストできますか?

**はい! Birouterは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.birouter`)
...
**はい、Birouterはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.birouter`(またはカスタム`DATA_DIR`)にローカル保存
- Birouterサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**破壊的変更:**
- [CHANGELOG.md](https://github.com/IQ-Kat/birouter/blob/main/CHANGELOG.md)を確認
- メジャー更新前に`~/.birouter`をバックアップ
- メジャーバージョンの移行ガイドを確認
...
詳細は[CONTRIBUTING.md](https://github.com/IQ-Kat/birouter/blob/main/CONTRIBUTING.md)を参照。

---

## さらにヘルプが必要?

- **ドキュメント:** [birouter.com/docs](https://birouter.com/docs)
