...
BirouterをOpenAI Codex CLIと統合し、OpenAI APIリクエストをBirouterのインテリジェントルーティングシステム経由でルーティングします。
...
1. BirouterダッシュボードでAPIキーが正しいか確認
...
1. モデル名がBirouter設定と一致するか確認
2. BirouterダッシュボードでOpenAIプロバイダー接続がアクティブか確認
3. 接続されたプロバイダーでモデルが利用可能か確認

## クラウドエンドポイント

localhostの代わりにBirouterクラウドエンドポイントを使用するには:

```bash
export OPENAI_BASE_URL="https://birouter.com"
```

BirouterクラウドダッシュボードでAPIキーが設定されていることを確認してください。
