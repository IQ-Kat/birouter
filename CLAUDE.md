...
**Birouter** — unified AI proxy/router. One endpoint, 237 LLM providers, auto-fallback.
...
Birouter has three related but distinct temporary-failure mechanisms. Keep their
...
- **Imports**: external → internal (`@/`, `@birouter/open-sse`) → relative
...
repo** instead of polluting the main Birouter tree.
...
- **Runtime**: Node.js ≥22.0.0 <23 || ≥24.0.0 <27, ES Modules. This is the **only** runtime for the published `birouter` CLI, the server, and the test suites (`node:test` + vitest) — `engines.node` is authoritative and end users never need Bun.
...
Birouter has **~48 quality-gate scripts** (`scripts/check/` + `scripts/quality/`) wired
