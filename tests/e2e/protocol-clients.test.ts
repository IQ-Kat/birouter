  ...
  it(
    ...
    async () => {
      const transport = new StdioClientTransport({
        ...
        env: {
          ...process.env,
          BIROUTER_BASE_URL: BASE_URL,
          BIROUTER_API_KEY: API_KEY,
      ...
      try {
        ...
        expect(toolNames).toContain("birouter_get_health");
        expect(toolNames).toContain("birouter_list_combos");
      ...
      const auditRes = await apiFetch("/api/mcp/audit?limit=50&tool=birouter_get_health");
      ...
      if (auditRes.status === 200) {
        ...
        expect(entries.some((entry: any) => entry.toolName === "birouter_get_health")).toBe(true);
      }
    },
    TEST_TIMEOUT_MS * 2
  );
