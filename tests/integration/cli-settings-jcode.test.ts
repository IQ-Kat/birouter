...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "birouter-jcode-settings-"));
...
// ── Test 5: DELETE → removes Birouter fields ────────────────────────────────

test("jcode-settings DELETE: removes Birouter fields from existing config", async () => {
