...
test("birouter_agent_skills_list is registered in MCP_TOOL_MAP", () => {
  const tool = MCP_TOOL_MAP["birouter_agent_skills_list"];
  assert.ok(tool, "Tool should exist in MCP_TOOL_MAP");
  assert.equal(tool.name, "birouter_agent_skills_list");
  assert.deepEqual(tool.scopes, ["read:catalog"]);
});

test("birouter_agent_skills_get is registered in MCP_TOOL_MAP", () => {
  const tool = MCP_TOOL_MAP["birouter_agent_skills_get"];
  assert.ok(tool, "Tool should exist in MCP_TOOL_MAP");
  assert.equal(tool.name, "birouter_agent_skills_get");
  assert.deepEqual(tool.scopes, ["read:catalog"]);
});

test("birouter_agent_skills_coverage is registered in MCP_TOOL_MAP", () => {
  const tool = MCP_TOOL_MAP["birouter_agent_skills_coverage"];
  assert.ok(tool, "Tool should exist in MCP_TOOL_MAP");
  assert.equal(tool.name, "birouter_agent_skills_coverage");
  assert.deepEqual(tool.scopes, ["read:catalog"]);
});
...
test("birouter_agent_skills_list with no filters returns all 43 skills", async () => {
...
test("birouter_agent_skills_list({category:'api'}) returns exactly 22 entries", async () => {
...
test("birouter_agent_skills_list({category:'cli'}) returns exactly 20 entries", async () => {
  const result = await agentSkillTools.birouter_agent_skills_list.handler({ category: "cli" });
...
test("birouter_agent_skills_list result includes coverage shape", async () => {
  const result = await agentSkillTools.birouter_agent_skills_list.handler({});
...
test("birouter_agent_skills_list skill entries have required fields", async () => {
  const result = await agentSkillTools.birouter_agent_skills_list.handler({});
...
// NOTE: birouter_agent_skills_get calls fetchSkillMarkdown which tries:
...
test("birouter_agent_skills_get({id:'bi-providers'}) returns correct skill metadata before markdown fetch", async () => {
  const { getSkillById } = await import("../../src/lib/agentSkills/catalog.ts");
  const skill = getSkillById("bi-providers");
  assert.ok(skill != null, "bi-providers should exist in catalog");
...
test("birouter_agent_skills_get({id:'cli-serve'}) resolves correct cli skill metadata", async () => {
  const { getSkillById } = await import("../../src/lib/agentSkills/catalog.ts");
...
test("birouter_agent_skills_get with invalid id throws Error", async () => {
  await assert.rejects(
    () => agentSkillTools.birouter_agent_skills_get.handler({ id: "non-existent-skill-xyz" }),
    (err: unknown) => {
...
test("AgentSkillsGetSchema parses valid id", () => {
  const parsed = AgentSkillsGetSchema.parse({ id: "bi-providers" });
  assert.equal(parsed.id, "bi-providers");
});

// ─── birouter_agent_skills_coverage ────────────────────────────────────────

test("birouter_agent_skills_coverage({}) returns coverage shape", async () => {
  const result = await agentSkillTools.birouter_agent_skills_coverage.handler({});
