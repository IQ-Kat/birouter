...
function withTempEnv(fn) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "birouter-bootstrap-test-"));
  ...
  try {
    fn({ tempRoot, tempCwd, tempHome, dataDir: path.join(tempHome, ".birouter") });
  } finally {
...
test("bootstrapEnv prefers ~/.birouter/.env over server.env", () => {
  withTempEnv(({ dataDir }) => {
