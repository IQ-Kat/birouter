...
test("optimizePackageImports excludes the internal @birouter/open-sse workspace (build-OOM guard)", async () => {
  // Regression guard: adding the internal `@birouter/open-sse` workspace to
  ...
  assert.ok(
    !list.includes("@birouter/open-sse"),
    "do NOT add the internal @birouter/open-sse workspace to optimizePackageImports — it OOMs the production build"
  );
