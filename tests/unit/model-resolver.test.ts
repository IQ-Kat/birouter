...
const modelResolverDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "birouter-model-resolver-"));
...
test(
  ...
  withEnv("BIROUTER_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS", "true", async () => {
...
test("getModelInfoCore routes unprefixed Claude models to Claude Code from settings toggle", async () => {
  const previousEnvFlag = process.env.BIROUTER_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS;
  delete process.env.BIROUTER_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS;
  ...
  try {
    ...
    if (previousEnvFlag === undefined) {
      ...
      process.env.BIROUTER_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS = previousEnvFlag;
    }
  }
});
...
test("getModelInfoCore lets settings toggle disable Claude Code preference", async () => {
  const previousEnvFlag = process.env.BIROUTER_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS;
  process.env.BIROUTER_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS = "true";
