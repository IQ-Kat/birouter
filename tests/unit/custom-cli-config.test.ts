...
test("normalizeOpenAiBaseUrl appends /v1 only when needed", () => {
  assert.equal(normalizeOpenAiBaseUrl("http://localhost:2004"), "http://localhost:2004/v1");
  assert.equal(
...
test("buildAliasEnvVar sanitizes aliases for env variable export", () => {
  assert.equal(buildAliasEnvVar("review"), "BIROUTER_MODEL_REVIEW");
  assert.equal(buildAliasEnvVar("plan mode"), "BIROUTER_MODEL_PLAN_MODE");
...
test("custom CLI generators include default model and alias mappings", () => {
  ...
  assert.match(envScript, /export OPENAI_BASE_URL="http:\/\/localhost:2004\/v1"/);
  assert.match(envScript, /export OPENAI_MODEL="birouter\/fast"/);
  assert.match(envScript, /export BIROUTER_MODEL_REVIEW="cc\/claude-sonnet-4-5-20250929"/);
  assert.match(envScript, /# http:\/\/localhost:2004\/v1\/chat\/completions/);
