 ...
 *
 * Plan 21 F8 — D16: birouter_memory_search reads retrievalStrategy from settings.
 *
...
test("birouter_memory_search: strategy=hybrid in DB → handler returns success", async () => {
  ...
  const handler = memoryTools.birouter_memory_search.handler;
...
test("birouter_memory_search: strategy=recent in DB → handler maps to exact, returns success", async () => {
  const db = core.getDbInstance();
...
test("birouter_memory_search: hardcoded fallback config has retrievalStrategy=exact", async () => {
