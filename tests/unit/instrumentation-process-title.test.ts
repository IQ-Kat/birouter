...
test("renameProcessTitle renames a bare 'next-server' title to 'birouter'", () => {
  assert.equal(renameProcessTitle("next-server"), "birouter");
});

test("renameProcessTitle preserves a version suffix after 'next-server'", () => {
  assert.equal(renameProcessTitle("next-server (v16.2.9)"), "birouter (v16.2.9)");
});
...
test("renameProcessTitle is idempotent when called again on an already-renamed title", () => {
  ...
  assert.equal(once, "birouter (v16.2.9)");
  assert.equal(twice, "birouter (v16.2.9)");
});
