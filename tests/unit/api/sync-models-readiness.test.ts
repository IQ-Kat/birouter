...
test("self-fetch retries with backoff and succeeds on attempt 3", async () => {
  ...
  const result = await selfFetchWithRetry("http://127.0.0.1:2004/api/providers/conn-1/models", {
    fetch: fetchMock,
    maxRetries: 5,
    backoffMs: 5,
...
test("self-fetch falls back to in-process route after maxRetries failures", async () => {
  ...
  const result = await selfFetchWithRetry("http://127.0.0.1:2004/api/providers/conn-2/models", {
    fetch: fetchMock,
    maxRetries: 3,
    backoffMs: 5,
    connectionId: "conn-2",
...
test("self-fetch returns HTTP error responses immediately without retrying", async () => {
  // 5xx HTTP response: server is up but returned error
  {
    ...
    const res = await selfFetchWithRetry("http://127.0.0.1:2004/api/providers/conn-3/models", {
      fetch: fetchMock,
      maxRetries: 5,
      backoffMs: 5,
      inProcessFallback: inProcMock,
  ...
  // 4xx HTTP response: also returned immediately without retry
  {
    ...
    const res = await selfFetchWithRetry("http://127.0.0.1:2004/api/providers/conn-4/models", {
      fetch: fetchMock,
      maxRetries: 5,
      backoffMs: 5,
      inProcessFallback: inProcMock,
...
test("selfFetchWithRetry with gate: 17 concurrent callers produce one probe + one fetch each", async () => {
  ...
  await Promise.all(
    Array.from({ length: 17 }, (_, i) =>
      selfFetchWithRetry("http://127.0.0.1:2004/api/providers/conn-" + i + "/models", {
        fetch: mockFetch,
        maxRetries: 3,
        backoffMs: 5,
      })
