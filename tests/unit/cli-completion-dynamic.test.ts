...
test("runCompletionCommand bash retorna 0 e string não-vazia", async () => {
  ...
  assert.ok(out.includes("birouter"), "bash script should mention birouter");
  assert.ok(out.includes("_birouter"), "bash script should define _birouter function");
});
...
test("runCompletionCommand fish retorna 0", async () => {
  ...
  const out = chunks.join("");
  assert.ok(out.includes("birouter"), "fish script should mention birouter");
});
...
test("completion scripts incluem combos/providers/models no cache dinamicamente", async () => {
  ...
  const out = chunks.join("");
  assert.ok(
    out.includes("completion-cache.json") || out.includes("birouter_get_cache"),
