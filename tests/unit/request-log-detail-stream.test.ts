...
test("status discrepancy shows both Birouter and provider statuses", () => {
  const html = renderToStaticMarkup(
    React.createElement(RequestLoggerDetail, {
      log: {
        status: 504,
  ...
  assert.notEqual(
    html.indexOf("Birouter returned 504"),
    -1,
    "Should indicate Birouter returned its own status"
  );
});
