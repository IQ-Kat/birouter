...
describe("transformToBirouter", () => {
  test("converts LiteLLM per-token pricing to Birouter per-million format", () => {
    const raw = {
      "openai/gpt-4o": {
    ...
    const result = transformToBirouter(raw);
...
describe("pricing merge precedence", () => {
  test("user overrides > synced > defaults conceptual order", () => {
    ...
    const synced = transformToBirouter(raw);
