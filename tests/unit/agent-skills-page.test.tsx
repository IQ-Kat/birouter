...
function makeSkill(overrides: Partial<AgentSkill> = {}): AgentSkill {
  return {
    ...
    rawUrl:
      "https://raw.githubusercontent.com/IQ-Kat/birouter/refs/heads/main/skills/bi-providers/SKILL.md",
    githubUrl: "https://github.com/IQ-Kat/birouter/blob/main/skills/bi-providers/SKILL.md",
    ...overrides,
  };
}
...
describe("AgentSkillsPageClient", () => {
  ...
  it("clicking a card triggers preview fetch after 200ms debounce", async () => {
    vi.useFakeTimers();
    const skills = make42Skills();
    const fetchMock = mockFetch(skills, FULL_COVERAGE, "# bi-skill-0 doc");
    ...
    const firstCard = container.querySelector(
      "[data-testid='skill-card-bi-skill-0']"
    ) as HTMLElement | null;
...
describe("SkillPreviewPane", () => {
  ...
  it("shows error state when skillId provided but markdown is empty string", async () => {
    ...
    const localRoot = createRoot(container);
    await act(async () => {
      localRoot.render(<SkillPreviewPane skillId="bi-providers" markdown="" loading={false} />);
    });
