...
test("Image URL base: non-local BIROUTER_BASE_URL remains a compatibility fallback", async () => {
  await withEnv({ BIROUTER_BASE_URL: "https://bi.example.com/v1" }, async () => {
    assert.equal(__derivePublicBaseUrlForTesting(null), "https://bi.example.com");
  });
});
...
test("Provider registry: chatgpt-web exposes the current ChatGPT Web model catalog", async () => {
  ...
  // Public Birouter ids stay in historical dot form even though ChatGPT's
...
test("Executor MODEL_MAP: Birouter IDs translate to ChatGPT backend slugs", async () => {
  reset();
  const m = installMockFetch();
  try {
    ...
    for (const [biId, expectedSlug] of cases) {
      ...
      const convIdx = m.calls.urls.findIndex((u) => u.endsWith("/backend-api/f/conversation"));
      const body = JSON.parse(m.calls.bodies[convIdx]);
      assert.equal(body.model, expectedSlug, `${biId} should map to ${expectedSlug}`);
    }
  } finally {
    m.restore();
  }
...
test("MODEL_MAP drift guard: every advertised catalog id reaches ChatGPT as a backend slug", async () => {
  ...
  try {
    for (const biId of ids) {
      ...
      assert.ok(
        !body.model.includes("."),
        `${biId} reached the backend as "${body.model}" (still dot-form)`
      );
      assert.equal(body.model, expectedSlugById[biId], `${biId} should map to backend slug`);
    }
  } finally {
    m.restore();
  }
...
test("Image gen: file-service:// pointer resolves to download URL and is appended as markdown (non-streaming)", async () => {
  ...
  try {
    ...
    // from the Birouter cache; clients see a stable /v1/chatgpt-web/image
...
test("Image gen: sediment:// pointer prefers /files/<id>/download over /attachment", async () => {
  ...
  try {
    ...
    // bytes that are cached behind the emitted Birouter image URL.
    assert.match(
      content,
...
test("Image gen: signed URL bytes are cached and exposed via /v1/chatgpt-web/image URL", async () => {
  ...
  // so we fetch the bytes, cache them locally, and emit an Birouter image URL.
  ...
  tls.__setTlsFetchOverrideForTesting(async (url, opts = {}) => {
    ...
    if (u.startsWith(downloadUrl)) {
      ...
      // back into bytes before putting the image in Birouter's cache.
      return {
        status: 200,
        headers: makeHeaders({ "Content-Type": "image/png" }),
...
test("Image edit: cached Birouter image URL continues the saved ChatGPT conversation", async () => {
  reset();
...
test("Image cache: byte cap evicts oldest before count cap kicks in", async () => {
  ...
  const original = process.env.BIROUTER_CGPT_WEB_IMAGE_CACHE_MAX_MB;
  process.env.BIROUTER_CGPT_WEB_IMAGE_CACHE_MAX_MB = "10";
  try {
    ...
    if (original == null) delete process.env.BIROUTER_CGPT_WEB_IMAGE_CACHE_MAX_MB;
    else process.env.BIROUTER_CGPT_WEB_IMAGE_CACHE_MAX_MB = original;
    cacheMod.__resetChatGptImageCacheForTesting();
  }
});
...
test("Image edit handler: no cached match returns 400 (does not silently generate unrelated image)", async () => {
  ...
  try {
    ...
    assert.match(String((result as any).error), /generated through this Birouter instance/);
