...
describe("GET /api/oauth/cursor/auto-import", () => {
  ...
  it("returns windowsManual/manual fallback if macOS db file exists but cannot be opened", async () => {
  ...
  it("returns windowsManual: true when tokens are missing", async () => {
    vi.mocked(fsPromises.access).mockResolvedValue();
    mockDbInstance.prepare.mockReturnValue({
  ...
  // ── Linux candidate probing ───────────────────────────────────────────

  it("linux probes candidates and returns checked locations on failure", async () => {
    ...
    expect(response.body.found).toBe(false);
    expect(response.body.error).toContain("Cursor database not found. Checked locations:");
    const normalizedError = response.body.error.replace(/\\/g, "/");
    expect(normalizedError).toContain(".config/Cursor/User");
    expect(normalizedError).toContain(".config/cursor/User");
  });
});
