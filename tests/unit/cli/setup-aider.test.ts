...
test("resolveAiderTarget strips /v1 (LiteLLM appends it)", () => {
  assert.equal(resolveAiderTarget({ remote: "http://vps:2004/v1/" }).apiBase, "http://vps:2004");
  assert.equal(resolveAiderTarget({ remote: "http://vps:2004" }).apiBase, "http://vps:2004");
});
test("resolveAiderTarget: explicit --api-key wins", () => {
  assert.equal(resolveAiderTarget({ remote: "http://x:2004", apiKey: "sk-x" }).apiKey, "sk-x");
});
test("buildAiderConfig sets openai-api-base + openai/<model>, preserves rest", () => {
  const c = buildAiderConfig(
    { "auto-commits": false },
    { apiBase: "http://vps:2004", model: "glm/glm-5.2" }
  );
  assert.equal(c["openai-api-base"], "http://vps:2004");
...
test("buildAiderRecipe references the env key + headless command", () => {
  const r = buildAiderRecipe({ apiBase: "http://vps:2004", model: "glm/glm-5.2" });
  assert.ok(r.includes("OPENAI_API_BASE=http://vps:2004"));
