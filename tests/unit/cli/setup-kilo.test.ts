...
test("buildKiloAuth sets the openai-compatible provider (baseUrl WITH /v1, model)", () => {
  const auth = buildKiloAuth(
    {},
    { apiKey: "sk-x", baseUrl: "http://vps:2004/v1", model: "glm/glm-5.2" }
  );
  assert.equal(auth["openai-compatible"].apiKey, "sk-x");
  assert.equal(auth["openai-compatible"].baseUrl, "http://vps:2004/v1");
...
test("buildKiloAuth falls back to a placeholder key", () => {
  ...
  assert.equal(auth["openai-compatible"].apiKey, "sk_birouter");
});

test("buildKiloVscodeSettings sets kilocode.customProvider + defaultModel, preserving others", () => {
  const s = buildKiloVscodeSettings(
    { "editor.fontSize": 14 },
    { apiKey: "k", baseUrl: "http://vps:2004/v1", model: "glm/glm-5.2" }
  );
  assert.equal(s["editor.fontSize"], 14);
  assert.equal(s["kilocode.customProvider"].name, "Birouter");
  assert.equal(s["kilocode.customProvider"].baseURL, "http://vps:2004/v1");
...
test("resolveKiloTarget: explicit --api-key wins", () => {
  assert.equal(
    resolveKiloTarget({ remote: "http://x:2004", apiKey: "sk-explicit" }).apiKey,
    "sk-explicit"
  );
});
