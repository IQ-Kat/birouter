...
test("buildProfileSettings pins the model + base URL + gateway discovery", () => {
  ...
  const json = JSON.parse(buildProfileSettings("glm/glm-5.2", "http://vps:2004", cfg));
  assert.equal(json.model, "glm/glm-5.2");
  assert.equal(json.env.ANTHROPIC_BASE_URL, "http://vps:2004");
...
test("buildProfileSettings NEVER writes the auth token to disk", () => {
  ...
  const raw = buildProfileSettings("kmc/kimi-k2.7", "http://vps:2004", cfg);
...
test("buildProfileSettings omits effortLevel for the simple tier", () => {
  ...
  const json = JSON.parse(buildProfileSettings("ollamacloud/gemma4:31b", "http://x:2004", cfg));
...
test("syncClaudeProfilesFromModels falls back to a generic profile for unmatched catalog models", async () => {
  const claudeHome = await fs.mkdtemp(path.join(os.tmpdir(), "birouter-claude-fallback-"));
  try {
    const result = await syncClaudeProfilesFromModels(
      [{ id: "new-provider/future-chat-1", output_modalities: ["text"] }],
      { claudeHome, baseUrl: "http://vps:2004" }
    );
...
test("syncClaudeProfilesFromModels dry-run writes nothing and reports via the injected log (#5959)", async () => {
  ...
  try {
    ...
    assert.equal(printed.env.ANTHROPIC_BASE_URL, "http://vps:2004");
...
test("buildClaudeEnv still accepts a bare port (backward compatible)", () => {
  const env = buildClaudeEnv({ PATH: "/bin" }, 2004, "secret");
  assert.equal(env.ANTHROPIC_BASE_URL, "http://localhost:2004");
...
test("buildClaudeEnv accepts a full base URL and strips /v1", () => {
  const env = buildClaudeEnv({}, "https://vps.example.com:2004/v1", "t");
  assert.equal(env.ANTHROPIC_BASE_URL, "https://vps.example.com:2004");
});

test("buildClaudeEnv sets CLAUDE_CONFIG_DIR for a profile", () => {
  const env = buildClaudeEnv({}, 2004, "t", { configDir: "/home/u/.claude/profiles/glm52" });
...
test("buildClaudeEnv strips inherited ANTHROPIC_* and does not mutate input", () => {
  ...
  const env = buildClaudeEnv(input, 2004, "x");
  assert.equal(env.ANTHROPIC_API_KEY, undefined);
  assert.equal(input.ANTHROPIC_API_KEY, "leak");
});

test("resolveLaunchTarget: explicit --remote wins, strips /v1", () => {
  const { baseUrl } = resolveLaunchTarget({ remote: "https://vps:2004/v1" });
  assert.equal(baseUrl, "https://vps:2004");
});

test("resolveLaunchTarget: explicit token wins over everything", () => {
  const { authToken } = resolveLaunchTarget({ remote: "http://x:2004", token: "tok-explicit" });
