...
test("buildQwenSettings de-dupes the birouter provider + preserves others", () => {
  const s = buildQwenSettings(
    { modelProviders: [{ id: "other" }, { id: "birouter", baseUrl: "old" }], theme: "dark" },
    { baseUrl: "http://x/v1", model: "m" }
  );
  assert.equal(s.modelProviders.filter((p) => p.id === "birouter").length, 1);
