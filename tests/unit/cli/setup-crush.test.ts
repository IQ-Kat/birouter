...
test("resolveCrushTarget ensures /v1", () => {
  assert.equal(resolveCrushTarget({ remote: "http://vps:2004" }).baseUrl, "http://vps:2004/v1");
});
test("resolveCrushTarget: explicit --api-key wins", () => {
  assert.equal(resolveCrushTarget({ remote: "http://x:2004", apiKey: "sk-x" }).apiKey, "sk-x");
});
test("buildCrushProvider emits openai-compat + env-ref key + curated models w/ context_window", () => {
  const p = buildCrushProvider(["glm/glm-5.2", "some/unknown"], "http://vps:2004/v1");
  assert.equal(p.type, "openai-compat");
  assert.equal(p.base_url, "http://vps:2004/v1");
  assert.equal(p.api_key, "$BIROUTER_API_KEY");
...
test("mergeCrushConfig adds providers.birouter, preserves the rest", () => {
  ...
  assert.ok(m.providers.birouter);
});
