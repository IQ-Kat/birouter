...
function withFixtureCli(files: Record<string, string>): { cleanup: () => void } {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "bi-cli-test-"));
...
export function registerKeys(program) {
  const keys = program.command('keys').description('Manage Birouter API keys');
...
test("parseCliRegistry() throws if commands directory is missing", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "bi-cli-missing-"));
  const originalCwd = process.cwd();
  process.chdir(tmpDir);
  try {
    assert.throws(
