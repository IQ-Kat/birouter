 ...
 * `globalThis.__biRequestHeaders` (set by the Next.js middleware shim) and
...
type BiGlobals = { __biRequestHeaders?: Headers };
const G = globalThis as BiGlobals;

function reset() {
  delete G.__biRequestHeaders;
}

function setHeaders(headers: Record<string, string>) {
  G.__biRequestHeaders = new Headers(headers);
}

test("isLocalRequestAllowed: allows when no headers injected and not production", () => {
  const prevNodeEnv = process.env.NODE_ENV;
  const prevEnabled = process.env.BIROUTER_LOCAL_ENDPOINTS_ENABLED;
  delete process.env.NODE_ENV;
  delete process.env.BIROUTER_LOCAL_ENDPOINTS_ENABLED;
  reset();
  try {
    ...
    if (prevEnabled !== undefined) process.env.BIROUTER_LOCAL_ENDPOINTS_ENABLED = prevEnabled;
  }
});
...
test("isLocalRequestAllowed: allows loopback host + empty xff (browser dev path)", () => {
  ...
  try {
    ...
    reset();
    if (prevNodeEnv !== undefined) process.env.NODE_ENV = prevNodeEnv;
    if (prevToken !== undefined) process.env.BIROUTER_LOCAL_ENDPOINTS_TOKEN = prevToken;
  }
});
...
test("isLocalRequestAllowed: bearer token takes precedence over host check (desktop app)", () => {
  const prevNodeEnv = process.env.NODE_ENV;
  const prevToken = process.env.BIROUTER_LOCAL_ENDPOINTS_TOKEN;
  process.env.BIROUTER_LOCAL_ENDPOINTS_TOKEN = "s3cret-token-abc";
  delete process.env.NODE_ENV;
