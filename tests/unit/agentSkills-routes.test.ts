...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "birouter-agentskills-routes-"));
...
test("GET /api/agent-skills/[id] — returns skill for valid id", async () => {
  const req = makeRequest("GET", "http://localhost/api/agent-skills/bi-providers");
  const res = await idRoute.GET(req, { params: Promise.resolve({ id: "bi-providers" }) });

  assert.equal(res.status, 200);
  const body = (await res.json()) as { id: string; category: string };
  assert.equal(body.id, "bi-providers");
...
test("GET /api/agent-skills/[id]/raw — returns markdown or 502 for valid id (no local file)", async () => {
  ...
  const req = makeRequest("GET", "http://localhost/api/agent-skills/bi-providers/raw");
  const res = await rawRoute.GET(req, { params: Promise.resolve({ id: "bi-providers" }) });
