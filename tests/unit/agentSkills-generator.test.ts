...
test("apply mode writes SKILL.md with valid frontmatter for bi-providers", async () => {
  const tmpDir = mkTmpDir();
  try {
    refreshCatalog();
    const report = await generateAgentSkills({
      ...
      prune: false,
      outputDir: tmpDir,
      onlyIds: ["bi-providers"],
    });

    assert.equal(report.errors.length, 0, `Errors: ${JSON.stringify(report.errors)}`);
    assert.equal(report.generated.length, 1);
    assert.equal(report.generated[0], "bi-providers");

    const skillFile = path.join(tmpDir, "bi-providers", "SKILL.md");
    ...
    assert.ok(content.includes("name: bi-providers"), "Missing name in frontmatter");
...
test("apply mode writes SKILL.md for an API skill with correct sections", async () => {
  ...
  try {
    ...
    assert.equal(report.errors.length, 0);
    const content = fs.readFileSync(path.join(tmpDir, "bi-auth", "SKILL.md"), "utf-8");
    assert.ok(content.includes("## Overview"), "Missing Overview section");
...
test("idempotency: second apply run reports 0 generated, all unchanged", async () => {
  ...
  try {
    ...
    const report1 = await generateAgentSkills({
      ...
      prune: false,
      outputDir: tmpDir,
      onlyIds: ["bi-providers", "cli-serve"],
    });
...
test("prune does not delete catalog skill dirs", async () => {
  ...
  try {
    ...
    const validDir = path.join(tmpDir, "bi-providers");
    ...
    // bi-providers should not be in orphansDetected
    assert.ok(
      !report.orphansDetected.includes("bi-providers"),
...
test("buildSkillMarkdown returns valid frontmatter + body for bi-providers", () => {
  refreshCatalog();
  const sources = emptySources();
  const result = buildSkillMarkdown("bi-providers", sources);

  assert.ok(typeof result.frontmatter === "object", "frontmatter must be an object");
  assert.equal(result.frontmatter.name, "bi-providers");
...
test("buildSkillMarkdown body has no erroneously escaped characters", () => {
  ...
  // Test several skills
  const testIds = ["bi-providers", "cli-serve", "bi-auth", "cli-health"];
...
test("buildSkillMarkdown API skill body contains expected sections", () => {
  refreshCatalog();
  const sources = emptySources();
  const result = buildSkillMarkdown("bi-cache", sources);
...
test("onlyIds filter limits generation to specified IDs", async () => {
  ...
  try {
    ...
    assert.ok(report.generated.includes("bi-providers"));
...
test("generated SKILL.md contains the mandatory generated comment", async () => {
  ...
  try {
    ...
    const content = fs.readFileSync(path.join(tmpDir, "bi-providers", "SKILL.md"), "utf-8");
    assert.ok(
      content.includes(
