...
BASE_IMAGE="${BASE_IMAGE:-birouter-local-base}"
CLI_IMAGE="${CLI_IMAGE:-birouter-local-cli}"

BASE_CONTAINER="${BASE_CONTAINER:-birouter-cli-runtime-base}"
CLI_CONTAINER="${CLI_CONTAINER:-birouter-cli-runtime-cli}"
HOST_CONTAINER="${HOST_CONTAINER:-birouter-cli-runtime-host}"
WRITE_BLOCK_CONTAINER="${WRITE_BLOCK_CONTAINER:-birouter-cli-runtime-write-block}"
WRITE_ALLOW_CONTAINER="${WRITE_ALLOW_CONTAINER:-birouter-cli-runtime-write-allow}"
REGRESSION_CONTAINER="${REGRESSION_CONTAINER:-birouter-cli-runtime-regression}"
...
docker build --target runner-base -t "${BASE_IMAGE}" "${WORKDIR}" >/tmp/birouter_cli_runtime_build_base.log
docker build --target runner-cli -t "${CLI_IMAGE}" "${WORKDIR}" >/tmp/birouter_cli_runtime_build_cli.log
echo "      Build done."

echo "[2/8] Validating runner-base (no CLIs)"
docker rm -f "${BASE_CONTAINER}" >/dev/null 2>&1 || true
docker run -d --name "${BASE_CONTAINER}" -p "${BASE_PORT}:2004" --env-file "${ENV_FILE}" "${BASE_IMAGE}" >/tmp/birouter_cli_runtime_base.cid
...
docker run -d --name "${CLI_CONTAINER}" -p "${CLI_PORT}:2004" --env-file "${ENV_FILE}" "${CLI_IMAGE}" >/tmp/birouter_cli_runtime_cli.cid
...
docker run -d --name "${HOST_CONTAINER}" -p "${HOST_PORT}:2004" \
  --env-file "${ENV_FILE}" \
  -e CLI_MODE=host \
  -e CLI_EXTRA_PATHS=/host-cli/bin \
  -v "${TMP_HOST_BIN_DIR}:/host-cli/bin:ro" \
  "${BASE_IMAGE}" >/tmp/birouter_cli_runtime_host.cid
...
docker run -d --name "${WRITE_BLOCK_CONTAINER}" -p "${WRITE_BLOCK_PORT}:2004" \
  --env-file "${ENV_FILE}" \
  -e CLI_ALLOW_CONFIG_WRITES=false \
  "${CLI_IMAGE}" >/tmp/birouter_cli_runtime_write_block.cid
wait_ready "${WRITE_BLOCK_PORT}" || { echo "      FAIL: write-block container did not become ready"; exit 1; }

WRITE_BLOCK_POST_CODE="$(
  curl -sS -o /tmp/birouter_cli_runtime_write_block_post.json -w '%{http_code}' \
    -X POST "http://127.0.0.1:${WRITE_BLOCK_PORT}/api/cli-tools/codex-settings" \
    -H 'Content-Type: application/json' \
    --data '{"baseUrl":"http://localhost:2004","apiKey":"sk_test_key","model":"cc/claude-opus-4-6"}'
)"
WRITE_BLOCK_DELETE_CODE="$(
  curl -sS -o /tmp/birouter_cli_runtime_write_block_delete.json -w '%{http_code}' \
...
WRITE_ALLOW_DELETE_CODE="$(
  curl -sS -o /tmp/birouter_cli_runtime_write_allow_delete.json -w '%{http_code}' \
...
docker run -d --name "${REGRESSION_CONTAINER}" -p "${REGRESSION_PORT}:2004" \
  --env-file "${ENV_FILE}" \
  -e CLI_CODEX_BIN=/host-bad/codex \
  -v "${TMP_BAD_BIN_DIR}:/host-bad:ro" \
  "${BASE_IMAGE}" >/tmp/birouter_cli_runtime_regression.cid
