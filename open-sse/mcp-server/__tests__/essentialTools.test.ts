 ...
 * The birouter_web_search tests use InMemoryTransport + Client to exercise
...
describe("MCP Essential Tools", () => {
  ...
  describe("Tool schema validation", () => {
    ...
    it("all tools should have birouter_ prefix", () => {
      const schemas = MCP_ESSENTIAL_TOOLS;
      for (const schema of schemas) {
        expect(schema.name).toMatch(/^birouter_/);
      }
    });
  });
  ...
  describe("get_health handler", () => {
    it("should return health data when API is available", async () => {
      ...
      const response = await mockFetch("http://localhost:2004/api/monitoring/health");
    ...
    it("should handle API failure gracefully", async () => {
      ...
      await expect(mockFetch("http://localhost:2004/api/monitoring/health")).rejects.toThrow();
    });
  });
  ...
  describe("check_quota handler", () => {
    it("should return quota data for all providers", async () => {
      ...
      const response = await mockFetch("http://localhost:2004/api/usage/quota");
      const data = await response.json();
    ...
    it("should filter by provider when specified", async () => {
      ...
      const response = await mockFetch("http://localhost:2004/api/usage/quota?provider=anthropic");
      const data = await response.json();
  ...
  describe("list_combos handler", () => {
    it("should return array of combos", async () => {
      ...
      const response = await mockFetch("http://localhost:2004/api/combos");
      const data = await response.json();
  ...
  describe("route_request handler", () => {
    it("should proxy chat completion request", async () => {
      ...
      const response = await mockFetch("http://localhost:2004/v1/chat/completions", {
        method: "POST",
  ...
  describe("cost_report handler", () => {
    it("should return cost analytics", async () => {
      ...
      const response = await mockFetch("http://localhost:2004/api/usage/analytics?period=session");
      const data = await response.json();
...
// ── birouter_web_search: handler dispatch tests ──────────────────────────────
...
describe("birouter_web_search handler (via MCP dispatch)", () => {
  let client: Client;
  ...
  it("should appear in tools/list after registration", async () => {
    ...
    const webSearch = tools.find((t) => t.name === "birouter_web_search");
