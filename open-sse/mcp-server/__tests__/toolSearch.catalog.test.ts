...
describe("getAllToolDefinitions", () => {
  ...
  it("aggregates many tools across collections", () => {
    ...
    expect(all.find((t) => t.name === "birouter_get_health")).toBeTruthy();
  });
  ...
  it("includes compressionTools-only entries (birouter_ccr_retrieve, not in MCP_TOOLS)", () => {
    // Regression: compressionTools carries birouter_ccr_retrieve, which is absent from
    ...
    expect(all.find((t) => t.name === "birouter_ccr_retrieve")).toBeTruthy();
  });
});
