...
export async function createVirtualAutoCombo(
  ...
  if (candidateFilter) {
    ...
    if (narrowed.length > 0) {
      ...
      log.warn(
        "AUTO",
        `${label} matched no connected models; falling back to the full pool (BIROUTER_AUTO_FREE_FALLBACK_TO_FULL_POOL=true)`
      );
    } else {
      ...
      log.warn(
        "AUTO",
        `${label} matched no connected models; returning an empty pool.${spec?.family ? "" : ' Set BIROUTER_AUTO_FREE_FALLBACK_TO_FULL_POOL=true to restore the legacy "use full pool" behavior.'}`
      );
      effectivePool = [];
    }
  }
